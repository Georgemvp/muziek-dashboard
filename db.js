...
function pruneCache({ maxRows = CACHE_MAX_ROWS, maxAgeMs = CACHE_MAX_AGE_MS } = {}) {
  try {
    ...
    if (Number.isFinite(maxAgeMs) && maxAgeMs > 0) {
      ...
      const stmt = db.prepare('DELETE FROM cache WHERE updated_at < ?');
...
function addToWishlist(type, name, artist, image) {
  try {
    ...
    let id;
    if (res.changes === 0) {
      id = db.prepare('SELECT id FROM wishlist WHERE type = ? AND name = ?').get(type, name).id;
...
function getDownloads() {
  try {
    const downloads = db.prepare('SELECT * FROM downloads ORDER BY queued_at DESC').all();
