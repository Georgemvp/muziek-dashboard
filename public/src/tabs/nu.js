    ...
    if (lfmRes.status === 'fulfilled') {
      ...
      if (np) {
        ...
        html += `<div class="w-np-row">
          ...
          ${img ? `<img class="w-np-img" src="${esc(img)}" alt="" loading="lazy">` : ''}
          <div class="w-np-info">
...
async function dw_nieuweReleases() {
  ...
  try {
    ...
    el.innerHTML = `<div class="w-releases-list">${recent.map(r => {
      const img = r.image
        ? `<img class="w-rel-img" src="${esc(r.image)}" alt="" loading="lazy" onerror="this.style.display='none'">`
...
async function dw_aanbeveling() {
  ...
  try {
    ...
    el.innerHTML = `<div class="w-rec-wrap">
      <div class="w-rec-top">
        ${img
          ? `<img class="w-rec-img" src="${esc(img)}" alt="" loading="lazy">`
...
export async function loadRecent() {
  ...
  try {
    ...
    for (const t of tracks) {
      ...
      const imgEl  = img
        ? `<img class="card-img" src="${esc(img)}" alt="" loading="lazy" onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="card-ph" style="display:none">♪</div>`
