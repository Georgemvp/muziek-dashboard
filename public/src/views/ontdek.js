...
export async function loadRecs() {
  ...
  try {
    ...
    if (albumRecs.length) {
      ...
      for (const a of albumRecs) {
        ...
        const imgEl = albrecImgSrc
          ? `<img class="albrec-img" src="${esc(albrecImgSrc)}" alt="${esc(a.album)} by ${esc(a.artist)}" loading="lazy" decoding="async"
               onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
        ...
        if (albEl) {
          ...
          if (albums.length) {
            ...
            for (const a of albums) {
              const imgEl = a.image
                ? `<img class="rec-album-img" src="${proxyImg(a.image, 48) || a.image}" alt="${esc(a.name)}" loading="lazy" decoding="async">` : `<div class="rec-album-ph">♪</div>`;
    ...
    // Use existing previewEl from above
    if (previewEl) {
      const previewItems = recs.slice(0, 8);
      const previewRequests = previewItems.map((r, i) =>
        apiFetch(`/api/artist/${encodeURIComponent(r.name)}/info`)
          .then(info => {
            const el = document.getElementById(`recs-thumb-${i}`);
            if (el && info.image) {
              el.innerHTML = `<img src="${esc(proxyImg(info.image, 48) || info.image)}" alt="${esc(r.name)}" loading="lazy" decoding="async" onerror="this.remove()">`;
            }
            return true;
          })
          .catch(() => true)
...
export function renderReleases() {
  ...
  for (const r of filtered) {
    ...
    const imgEl = r.image
      ? `<img class="rel-img" src="${esc(r.image)}" alt="${esc(r.album)} by ${esc(r.artist)}" loading="lazy" decoding="async"
           onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
  ...
  if (previewEl) {
    ...
    previewEl.innerHTML = `<div class="collapsed-thumbs">${previewItems.map(r => {
      if (r.image) return `<div class="collapsed-thumb">
          <img src="${esc(r.image)}" alt="${esc(r.album)} by ${esc(r.artist)}" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
...
export function renderDiscover() {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    const photo = discImgSrc
      ? `<img class="discover-photo" src="${esc(discImgSrc)}" alt="${esc(a.name)}" loading="lazy" decoding="async"
      ...
      <div class="discover-section collapsed" id="disc-${i}">
        <div class="discover-card discover-card-toggle" data-disc-id="disc-${i}">
          ...
          ${a.albums?.length ? `<div class="discover-preview-row">${a.albums.slice(0, 5).map(alb => {
            ...
            return alb.coverUrl
              ? `<img class="discover-preview-thumb" src="${esc(alb.coverUrl)}" alt="${esc(alb.title)}${alb.year ? ' ('+alb.year+')' : ''}" loading="lazy" decoding="async"
  ...
  if (previewEl) {
    ...
    previewEl.innerHTML = `<div class="collapsed-thumbs">${previewItems.map(a => {
      if (a.image) return `<div class="collapsed-thumb collapsed-thumb-round">
          <img src="${esc(a.image)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
  ...
  (async () => {
    try {
      ...
      if (previewEl && filtered.length) {
        ...
        previewEl.innerHTML = `<div class="collapsed-thumbs">${previewItems.map(a => {
          if (a.image) return `<div class="collapsed-thumb collapsed-thumb-round">
              <img src="${esc(a.image)}" alt="${esc(a.name)}" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
              <span class="collapsed-thumb-ph" style="display:none;background:${gradientFor(a.name)}">${initials(a.name)}</span>
            </div>`;
          return `<div class="collapsed-thumb collapsed-thumb-round" style="background:${gradientFor(a.name)}"><span class="collapsed-thumb-ph">${initials(a.name)}</span></div>`;
        }).join('')}${filtered.length > 8 ? `<span class="collapsed-thumbs-more">+${filtered.length - 8}</span>` : ''}</div>`;
