(()=>{var s={currentTab:"nu",currentMainTab:"nu",bibSubTab:"collectie",sectionContainerEl:null,currentPeriod:"7day",recsFilter:"all",discFilter:"all",gapsSort:"missing",releasesSort:"listening",releasesFilter:"all",plexOk:!1,lastDiscover:null,lastGaps:null,lastReleases:null,plexLibData:null,wishlistMap:new Map,newReleaseIds:new Set,searchTimeout:null,tidalSearchTimeout:null,tidarrOk:!1,tidalView:"search",tidalSearchResults:null,tidarrQueuePoll:null,tidarrSseSource:null,tidarrQueueItems:[],downloadedSet:new Set,spotifyEnabled:!1,activeMood:null,previewAudio:new Audio,previewBtn:null,collapsibleSections:{recs:!1,releases:!1,discover:!1},sectionMutex:Promise.resolve(),dlResolve:null,VALID_QUALITIES:["max","high","normal","low"]};var ye=window.matchMedia("(prefers-reduced-motion: reduce)").matches,T=document.getElementById("content");function I(e,t=120){return e?`/api/img?url=${encodeURIComponent(e)}&w=${t}&h=${t}`:null}var j=(e,t="medium")=>{if(!e)return null;let a=e.find(i=>i.size===t);return a&&a["#text"]&&!a["#text"].includes("2a96cbd8b46e442fc41c2b86b821562f")?a["#text"]:null},m=e=>String(e||"?").split(/\s+/).map(t=>t[0]).join("").toUpperCase().slice(0,2),B=e=>parseInt(e).toLocaleString("nl-NL"),l=e=>String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Y=e=>({"7day":"week","1month":"maand","3month":"3 maanden","12month":"jaar",overall:"alles"})[e]||e;function Z(e){let t=Math.floor(Date.now()/1e3)-e;return t<120?"zojuist":t<3600?`${Math.floor(t/60)}m`:t<86400?`${Math.floor(t/3600)}u`:`${Math.floor(t/86400)}d`}function g(e,t=!1){let a=0;for(let r=0;r<e.length;r++)a=a*31+e.charCodeAt(r)&16777215;let i=a%360,n=45+a%31,d=50+(a>>8)%26,c=20+(a>>16)%16,o=15+(a>>10)%11;return t?`radial-gradient(circle, hsl(${i},${n}%,${c}%), hsl(${(i+40)%360},${d}%,${o}%))`:`linear-gradient(135deg, hsl(${i},${n}%,${c}%), hsl(${(i+40)%360},${d}%,${o}%))`}function _(e){return!e||e.length!==2?"":[...e.toUpperCase()].map(t=>String.fromCodePoint(t.charCodeAt(0)+127397)).join("")}function O(e,t=4){return e?.length?`<div class="tags" style="margin-top:5px">${e.slice(0,t).map(a=>`<span class="tag">${l(a)}</span>`).join("")}</div>`:""}function X(e){return s.plexOk?e?'<span class="badge plex">\u25B6 In Plex</span>':'<span class="badge new">\u2726 Nieuw</span>':""}function H(e,t,a="",i=""){let n=s.wishlistMap.has(`${e}:${t}`);return`<button class="bookmark-btn${n?" saved":""}"
    ...
    <div class="album-card ${i?"owned":"missing"}" title="${l(e.title)}${d!=="\u2014"?" ("+d+")":""}">
    ...
    </div>`}function je(){if(ye)return;Object.entries({".rec-grid > *":60,".card-list > *":25,".artist-grid > *":40,".releases-grid > *":40,".wishlist-grid > *":40}).forEach(([t,a])=>{document.querySelectorAll(t).forEach((i,n)=>{i.style.animationDelay=`${n*a}ms`})})}function ct(e){let t="";e==="cards"?t='<div class="skeleton-list">'+Array(6).fill('<div class="skeleton skeleton-card"></div>').join("")+"</div>":e==="grid"?t='<div class="skeleton-grid">'+Array(8).fill('<div class="skeleton skeleton-square"></div>').join("")+"</div>":e==="stats"?t='<div class="skeleton-stats"><div class="skeleton skeleton-stat-full"></div><div class="skeleton-two"><div class="skeleton skeleton-stat-half"></div><div class="skeleton skeleton-stat-half"></div></div></div>':t=`<div class="loading"><div class="spinner"></div>${e||"Laden..."}</div>`,b(t)}function b(e,t){let a=s.sectionContainerEl||T;!s.sectionContainerEl&&document.startViewTransition?document.startViewTransition(()=>{a.innerHTML=e,je(),t&&requestAnimationFrame(t)}).finished.catch(()=>{}):(a.innerHTML=e,s.sectionContainerEl?t&&t():(T.style.opacity="0",T.style.transform="translateY(6px)",requestAnimationFrame(()=>{T.offsetHeight,T.style.opacity="1",T.style.transform="",je(),t&&requestAnimationFrame(t)})))}var C=e=>b(`<div class="error-box">\u26A0\uFE0F ${l(e)}</div>`);function q(e){if(s.sectionContainerEl){s.sectionContainerEl.innerHTML=`<div class="loading"><div class="spinner"></div>${e||"Laden..."}</div>`;return}let a={recent:"cards",loved:"cards",toptracks:"cards",topartists:"grid",releases:"grid",recs:"grid",discover:"grid",gaps:"grid",stats:"stats",wishlist:"grid",plexlib:"cards",nu:"cards",ontdek:"grid",bibliotheek:"cards",downloads:"cards"}[s.currentTab];a&&!e?ct(a):b(`<div class="loading"><div class="spinner"></div>${e||"Laden..."}</div>`)}function ee(e,t){if(!e)return;if(!("IntersectionObserver"in window)){t();return}let a=new IntersectionObserver(i=>{i.forEach(n=>{n.isIntersecting&&(a.unobserve(n.target),t())})},{rootMargin:"300px"});a.observe(e)}function R(e,t){let a=s.sectionMutex.then(async()=>{s.sectionContainerEl=e;try{await t()}finally{s.sectionContainerEl=null}});return s.sectionMutex=a.catch(()=>{}),a}function ut(e){if(document.getElementById("rate-limit-notice"))return;let t=document.createElement("div");t.id="rate-limit-notice",Object.assign(t.style,{position:"fixed",top:"16px",left:"50%",transform:"translateX(-50%)",background:"#e05a2b",color:"#fff",padding:"12px 24px",borderRadius:"8px",zIndex:"9999",fontSize:"14px",fontFamily:"sans-serif",boxShadow:"0 4px 16px rgba(0,0,0,0.35)",whiteSpace:"nowrap"}),t.textContent="\u23F1 "+e,document.body.appendChild(t),setTimeout(()=>t.remove(),8e3)}async function p(e){let t=await fetch(e);if(t.status===429){let i=(await t.json().catch(()=>({}))).error||"Te veel verzoeken, probeer het over een minuut opnieuw";throw ut(i),new Error(i)}if(!t.ok)throw new Error(`Serverfout ${t.status}`);return t.json()}async function Fe(){try{let e=await p("/api/downloads/keys");s.downloadedSet=new Set(e)}catch{s.downloadedSet=new Set}}async function N(){try{let e=await fetch("/api/plex/status").then(i=>i.json()),t=document.getElementById("plex-pill"),a=document.getElementById("plex-pill-text");if(e.connected){s.plexOk=!0,t.className="plex-pill on";let i=e.albums?` \xB7 ${B(e.albums)} albums`:"";a.textContent=`Plex \xB7 ${B(e.artists)} artiesten${i}`}else t.className="plex-pill off",a.textContent="Plex offline"}catch{document.getElementById("plex-pill-text").textContent="Plex offline"}}async function Oe(){try{let t=(await p("/api/user")).user,a=j(t.image,"large"),i=a?`<img class="user-avatar" src="${a}" alt="">`:`<div class="user-avatar-ph">${(t.name||"U")[0].toUpperCase()}</div>`,n=new Date(parseInt(t.registered?.unixtime)*1e3).getFullYear();document.getElementById("user-wrap").innerHTML=`
      ...
      </div>`}catch{}}async function we(){try{let e=await p("/api/wishlist");s.wishlistMap.clear();for(let t of e)s.wishlistMap.set(`${t.type}:${t.name}`,t.id);te()}catch{}}function te(){let e=document.getElementById("badge-wishlist");e&&(e.textContent=s.wishlistMap.size||"0")}async function Ne(e,t,a,i){let n=`${e}:${t}`;if(s.wishlistMap.has(n))return await fetch(`/api/wishlist/${s.wishlistMap.get(n)}`,{method:"DELETE"}),s.wishlistMap.delete(n),te(),!1;{let c=await(await fetch("/api/wishlist",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:e,name:t,artist:a,image:i})})).json();return s.wishlistMap.set(n,c.id),te(),!0}}async function se(){q(),await we();try{let e=await p("/api/wishlist");if(!e.length){b('<div class="empty">Je lijst is leeg.<br>Voeg artiesten toe via het \u{1F516} icoon in Ontdek en Collectiegaten.</div>');return}let t=`<div class="section-title">${e.length} opgeslagen</div><div class="wishlist-grid">`;for(let a of e){let i=a.image?`<img src="${l(a.image)}" alt="" loading="lazy"
    ...
    <div class="q-list">${t.map(n=>{let d=i[n.status]||"q-pending",c=a[n.status]||n.status||"In wachtrij",o=n.progress?.current&&n.progress?.total?Math.round(n.progress.current/n.progress.total*100):null,r=o!==null?`<div class="q-bar"><div class="q-bar-fill" style="width:${o}%"></div></div><div class="q-pct">${o}%</div>`:"";return`<div class="q-row">
      ...
      <div class="q-list">${t.map(i=>{let n=i.queued_at?new Date(i.queued_at).toLocaleDateString("nl-NL",{day:"numeric",month:"short",year:"numeric"}):"",d=a[i.quality]||i.quality||"";return`<div class="q-row">
        ...
        </div>`}).join("")}</div>`,document.getElementById("dl-history-clear")?.addEventListener("click",async()=>{if(confirm("Wis de volledige download-geschiedenis?")){await fetch("/api/downloads",{method:"DELETE"}).catch(()=>{});for(let i of t)await fetch(`/api/downloads/${i.id}`,{method:"DELETE"}).catch(()=>{});s.downloadedSet.clear(),Qe()}})}catch(t){e.innerHTML=`<div class="error-box">\u26A0\uFE0F ${l(t.message)}</div>`}}}function de(e){s.tidalView=e,document.querySelectorAll("[data-tidal-view]").forEach(t=>t.classList.toggle("sel-def",t.dataset.tidalView===e)),e==="search"?xe(document.getElementById("tidal-search")?.value||""):e==="queue"?Ue():e==="history"&&Qe()}function ce(){if(s.tidarrSseSource)return;let e=new EventSource("/api/tidarr/stream");s.tidarrSseSource=e,e.onmessage=t=>{try{s.tidarrQueueItems=JSON.parse(t.data)||[]}catch{s.tidarrQueueItems=[]}let a=s.tidarrQueueItems.filter(n=>n.status!=="finished"&&n.status!=="error"),i=[document.getElementById("badge-tidarr-queue"),document.getElementById("badge-tidarr-queue-inline")];for(let n of i)n&&(a.length>0?(n.textContent=a.length,n.style.display=""):n.style.display="none");if(vt(s.tidarrQueueItems),s.currentTab==="tidal"&&s.tidalView==="queue"&&Ue(),document.getElementById("queue-popover")?.classList.contains("open")&&Je(),s.currentTab==="nu"){let n=document.getElementById("wbody-download-voortgang");n&&Ee(n,a)}},e.onerror=()=>{e.close(),s.tidarrSseSource=null,setTimeout(ce,1e4)}}function Ee(e,t){if(t||(t=s.tidarrQueueItems.filter(i=>i.status!=="finished"&&i.status!=="error")),!t.length){e.innerHTML='<div class="empty" style="font-size:12px">Geen actieve downloads</div>';return}let a={queue_download:"In wachtrij",queue_processing:"Verwerken",download:"Downloaden\u2026",processing:"Verwerken\u2026"};e.innerHTML=`<div class="w-queue-list">${t.slice(0,5).map(i=>{let n=i.progress?.current&&i.progress?.total?Math.round(i.progress.current/i.progress.total*100):null;return`<div class="w-q-row"><div class="w-q-info">
    ...
    </div></div>`}).join("")}${t.length>5?`<div style="font-size:11px;color:var(--muted2);margin-top:6px">+${t.length-5} meer</div>`:""}</div>`}function mt(){ce()}function We(){let e=document.getElementById("tidarr-iframe"),t=document.getElementById("tidarr-ui-wrap"),a=document.getElementById("content");t.style.display="flex",a.style.display="none",e.dataset.loaded||(e.src=e.dataset.src,e.dataset.loaded="1")}function D(){document.getElementById("tidarr-ui-wrap").style.display="none",document.getElementById("content").style.display=""}function vt(e){let t=document.getElementById("queue-fab"),a=document.getElementById("fab-queue-badge");if(!t)return;let i=(e||[]).filter(n=>n.status!=="finished"&&n.status!=="error");e&&e.length>0?(t.style.display="",i.length>0?(a.textContent=i.length,a.style.display=""):a.style.display="none"):(t.style.display="none",document.getElementById("queue-popover")?.classList.remove("open"))}function Je(){let e=document.getElementById("queue-popover-list");if(!e)return;let t=s.tidarrQueueItems;if(!t.length){e.innerHTML='<div class="qpop-empty">Queue is leeg</div>';return}let a={queue_download:"In wachtrij",queue_processing:"Verwerken",download:"Downloaden\u2026",processing:"Verwerken\u2026",finished:"Klaar \u2713",error:"Fout"},i={queue_download:"q-pending",queue_processing:"q-pending",download:"q-active",processing:"q-active",finished:"q-done",error:"q-error"};e.innerHTML=t.map(n=>{let d=i[n.status]||"q-pending",c=a[n.status]||n.status||"In wachtrij",o=n.progress?.current&&n.progress?.total?Math.round(n.progress.current/n.progress.total*100):null,r=o!==null?`<div class="q-bar" style="margin-top:4px"><div class="q-bar-fill" style="width:${o}%"></div></div>`:"";return`<div class="qpop-row">
      <div class="qpop-title">${l(n.title||"(onbekend)")}</div>
      ${n.artist?`<div class="qpop-artist">${l(n.artist)}</div>`:""}
      <span class="q-status ${d}">${l(c)}</span>
      ${r}
    </div>`}).join("")}function gt(){let e=document.getElementById("queue-popover");if(!e)return;e.classList.toggle("open")&&Je()}function ke(){document.getElementById("queue-popover")?.classList.remove("open")}function _e(e){return(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g,"")}function Ye(e,t){let a=_e(e),i=_e(t);return!a||!i?!0:a===i||a.includes(i)||i.includes(a)}function ft(e,t,a,i){return new Promise(n=>{s.dlResolve=n;let d=document.getElementById("dl-confirm-modal"),c=document.getElementById("dl-confirm-cards");document.getElementById("dl-confirm-wanted").textContent=`"${a}"${t?" \u2013 "+t:""}`,c.innerHTML=e.map((o,r)=>{let u=!Ye(o.artist,t),f=o.image?`<img class="dlc-img" src="${l(o.image)}" alt="" loading="lazy"
             onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
           <div class="dlc-ph" style="display:none">${m(o.title)}</div>`:`<div class="dlc-ph">${m(o.title)}</div>`,y=u?`<div class="dlc-artist dlc-artist-warn">\u26A0 ${l(o.artist)}</div>`:`<div class="dlc-artist">${l(o.artist)}</div>`,w=o.score??0;return`
        ...
        </button>`}).join(""),c.querySelectorAll(".dlc-card").forEach(o=>{o.addEventListener("click",()=>{let r=parseInt(o.dataset.dlcIdx);Le(),n({chosen:e[r],btn:i})})}),d.classList.add("open"),document.body.style.overflow="hidden"})}function Le(){document.getElementById("dl-confirm-modal")?.classList.remove("open"),document.body.style.overflow="",s.dlResolve&&(s.dlResolve({chosen:null}),s.dlResolve=null)}async function Ge(e,t,a,i){let n=await fetch("/api/tidarr/download",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:e.url,type:e.type||"album",title:e.title||a||"",artist:e.artist||t||"",id:String(e.id||""),quality:pt()})}),d=await n.json();if(!n.ok||!d.ok)throw new Error(d.error||"download mislukt");Re(e.artist||t||"",e.title||a||""),i&&(i.textContent="\u2713",i.classList.add("dl-done"),i.disabled=!1),await re()}async function Ze(e,t,a){if(!s.tidarrOk){alert("Tidarr is niet verbonden. Controleer TIDARR_URL en TIDARR_API_KEY.");return}a&&(a.disabled=!0,a.textContent="\u2026");try{let i=new URLSearchParams;e&&i.set("artist",e),t&&i.set("album",t);let n=await fetch(`/api/tidarr/candidates?${i}`);if(!n.ok){n.status===401?alert(`Niet ingelogd bij TIDAL.
Ga naar de \u{1F39B}\uFE0F Tidarr-tab en koppel je TIDAL-account eerst.`):alert(`Niet gevonden op TIDAL: "${t}"${e?" van "+e:""}

Probeer het handmatig via de \u{1F30A} Tidal-tab.`),a&&(a.disabled=!1,a.textContent="\u2B07");return}let{candidates:d}=await n.json();if(!d?.length){alert(`Niet gevonden op TIDAL: "${t}"${e?" van "+e:""}`),a&&(a.disabled=!1,a.textContent="\u2B07");return}let c=d[0];if(e&&!Ye(c.artist,e)){a&&(a.disabled=!1,a.textContent="\u2B07");let{chosen:o}=await ft(d,e,t,a);if(!o)return;a&&(a.disabled=!0,a.textContent="\u2026"),await Ge(o,e,t,a)}else await Ge(c,e,t,a)}catch(i){alert("Downloaden mislukt: "+i.message),a&&(a.disabled=!1,a.textContent="\u2B07")}}async function Ie(){b('<div id="tidal-content"><div class="empty">Begin met typen om te zoeken op Tidal.</div></div>'),await $e(),await re(),de(s.tidalView),mt()}function Xe(){s.currentTab="tidal",D(),document.getElementById("tb-tidal")?.classList.add("visible"),Ie()}document.getElementById("dl-confirm-cancel")?.addEventListener("click",()=>{Le()});document.getElementById("dl-confirm-modal")?.addEventListener("click",e=>{e.target===document.getElementById("dl-confirm-modal")&&Le()});document.getElementById("queue-fab")?.addEventListener("click",gt);document.getElementById("qpop-close")?.addEventListener("click",e=>{e.stopPropagation(),ke()});document.getElementById("qpop-goto-tidal")?.addEventListener("click",()=>{ke(),document.querySelector('.tab[data-tab="downloads"]')?.click(),setTimeout(()=>de("queue"),150)});document.addEventListener("click",e=>{let t=document.getElementById("queue-popover"),a=document.getElementById("queue-fab");t?.classList.contains("open")&&!t.contains(e.target)&&!a?.contains(e.target)&&ke()},!0);document.getElementById("btn-tidarr-reload")?.addEventListener("click",()=>{let e=document.getElementById("tidarr-iframe");e.src=e.dataset.src});async function et(){try{let e=await p("/api/spotify/status");s.spotifyEnabled=!!e.enabled;let t=document.getElementById("tb-mood");s.spotifyEnabled&&s.currentTab==="recs"?(t.style.display="",t.classList.add("visible")):s.spotifyEnabled&&(t.style.display="")}catch{s.spotifyEnabled=!1}}function bt(e,t){let a=e.image?`<img src="${l(e.image)}" alt="" loading="lazy"
         onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
    ...
    <div class="spotify-card">
      ...
      ${s.plexOk?` &nbsp;\xB7&nbsp; <span style="color:var(--new)">${n} nieuw</span> \xB7 <span style="color:var(--plex)">${d} in Plex</span>`:""}
        ...
        <div class="albrec-grid">`;for(let u of a){let f=I(u.image,80)||u.image,y=f?`<img class="albrec-img" src="${l(f)}" alt="" loading="lazy"
               onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
        ...
        <div class="trackrec-list">`;for(let u of i){let f=u.playcount>0?`<span class="trackrec-plays">${B(u.playcount)}\xD7</span>`:"",y=u.url?`<a class="trackrec-link" href="${l(u.url)}" target="_blank" rel="noopener">Last.fm \u2197</a>`:"";o+=`
          <div class="trackrec-row">
            ...
            <div class="trackrec-meta">${f}${y}</div>
          </div>`}o+="</div>"}b(o,()=>{s.activeMood&&pe(s.activeMood)}),me();let r=document.getElementById("sec-recs-preview");if(r){let u=t.slice(0,8);r.innerHTML=`<div class="collapsed-thumbs">${u.map((f,y)=>`<div class="collapsed-thumb collapsed-thumb-round" id="recs-thumb-${y}" style="background:${g(f.name)}">
          <span class="collapsed-thumb-ph">${m(f.name)}</span>
        </div>`).join("")}${t.length>8?`<span class="collapsed-thumbs-more">+${t.length-8}</span>`:""}</div>`,u.forEach(async(f,y)=>{try{let w=await p(`/api/artist/${encodeURIComponent(f.name)}/info`),E=document.getElementById(`recs-thumb-${y}`);E&&w.image&&(E.innerHTML=`<img src="${l(I(w.image,48)||w.image)}" alt="" loading="lazy" onerror="this.remove()">`)}catch{}})}t.forEach(async(u,f)=>{try{let y=await p(`/api/artist/${encodeURIComponent(u.name)}/info`),w=document.getElementById(`rph-${f}`);w&&y.image&&(w.innerHTML=`<img src="${I(y.image,120)||y.image}" alt="" loading="lazy"
          onerror="this.parentElement.innerHTML='<div class=\\'rec-photo-ph\\' style=\\'background:${g(u.name)}\\'>${m(u.name)}</div>'">`);let E=document.getElementById(`rtags-${f}`);E&&(E.innerHTML=O(y.tags,3)+'<div style="height:6px"></div>');let $=document.getElementById(`ralb-${f}`);if($){let x=(y.albums||[]).slice(0,4);if(x.length){let A='<div class="rec-albums-label">Bekende albums</div><div class="rec-albums-list">';for(let M of x){let v=M.image?`<img class="rec-album-img" src="${I(M.image,48)||M.image}" alt="" loading="lazy">`:'<div class="rec-album-ph">\u266A</div>',h=s.plexOk&&M.inPlex?'<span class="rec-album-plex">\u25B6</span>':"";A+=`<div class="rec-album-row">${v}<span class="rec-album-name">${l(M.name)}</span>${h}${z(u.name,M.name,M.inPlex)}</div>`}$.innerHTML=A+"</div>"}else $.innerHTML=""}}catch{let y=document.getElementById(`ralb-${f}`);y&&(y.innerHTML="")}})}catch(e){C(e.message)}}function me(){document.querySelectorAll(".rec-card[data-inplex]").forEach(e=>{let t=e.dataset.inplex==="true",a=!0;s.recsFilter==="new"&&(a=!t),s.recsFilter==="plex"&&(a=t),e.classList.toggle("hidden",!a)})}function tt(e){let t=document.getElementById("badge-releases");t&&(e>0?(t.textContent=e,t.style.display=""):t.style.display="none")}function yt(e){if(!e)return"";let t=new Date(e),i=Math.floor((new Date-t)/864e5);return i===0?"vandaag":i===1?"gisteren":i<7?`${i} dagen geleden`:t.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})}async function G(){q();try{let e=await p("/api/releases");if(e.status==="building"){b(`<div class="loading"><div class="spinner"></div>
        <div>${l(e.message)}</div>
        <div class="build-hint">Pagina ververst automatisch over 5 seconden</div></div>`),setTimeout(()=>{(s.currentTab==="releases"||s.currentTab==="ontdek")&&G()},5e3);return}s.lastReleases=e.releases||[],s.newReleaseIds=new Set(e.newReleaseIds||[]),tt(e.newCount||0),F()}catch(e){C(e.message)}}function F(){let e=s.lastReleases||[];if(!e.length){b('<div class="empty">Geen recente releases gevonden (afgelopen 30 dagen).</div>');return}let t=e;if(s.releasesFilter!=="all"&&(t=e.filter(o=>(o.type||"album").toLowerCase()===s.releasesFilter)),!t.length){b(`<div class="empty">Geen ${s.releasesFilter==="ep"?"EP's":s.releasesFilter+"s"} gevonden voor dit filter.</div>`);return}s.releasesSort==="listening"?t=[...t].sort((o,r)=>(r.artistPlaycount||0)-(o.artistPlaycount||0)||new Date(r.releaseDate)-new Date(o.releaseDate)):t=[...t].sort((o,r)=>new Date(r.releaseDate)-new Date(o.releaseDate));let a=document.getElementById("hdr-title-releases");a&&(a.textContent=`\u{1F4BF} Nieuwe Releases \xB7 ${t.length} release${t.length!==1?"s":""}`);let i=o=>({album:"Album",single:"Single",ep:"EP"})[o?.toLowerCase()]||o||"Album",n=o=>({album:"rel-type-album",single:"rel-type-single",ep:"rel-type-ep"})[o?.toLowerCase()]||"rel-type-album",d=`<div class="section-title">${t.length} release${t.length!==1?"s":""} in de afgelopen 30 dagen</div>
    <div class="releases-grid">`;for(let o of t){let r=s.newReleaseIds.has(`${o.artist}::${o.album}`),u=o.image?`<img class="rel-img" src="${l(o.image)}" alt="" loading="lazy"
      ...
      <div class="rel-card${r?" rel-card-new":""}">
      ...
      </div>`}b(d+"</div>");let c=document.getElementById("sec-releases-preview");if(c){let o=t.slice(0,8);c.innerHTML=`<div class="collapsed-thumbs">${o.map(r=>r.image?`<div class="collapsed-thumb">
        ...
        </div>`:`<div class="collapsed-thumb" style="background:${g(r.album)}"><span class="collapsed-thumb-ph">${m(r.album)}</span></div>`).join("")}${t.length>8?`<span class="collapsed-thumbs-more">+${t.length-8}</span>`:""}</div>`}}async function U(){q("Ontdekkingen ophalen...");try{let e=await p("/api/discover");if(e.status==="building"){b(`<div class="loading"><div class="spinner"></div>
        <div>${l(e.message)}</div>
        <div class="build-hint">Pagina ververst automatisch over 20 seconden</div></div>`),setTimeout(()=>{(s.currentTab==="discover"||s.currentTab==="ontdek")&&U()},2e4);return}s.lastDiscover=e,e.plexConnected&&(s.plexOk=!0),ae()}catch(e){C(e.message)}}function ae(){if(!s.lastDiscover)return;let{artists:e,basedOn:t}=s.lastDiscover;if(!e?.length){b('<div class="empty">Geen ontdekkingen gevonden.</div>');return}let a=e;if(s.discFilter==="new"&&(a=e.filter(o=>!o.inPlex)),s.discFilter==="partial"&&(a=e.filter(o=>o.inPlex&&o.missingCount>0)),!a.length){b('<div class="empty">Geen artiesten voor dit filter.</div>');return}let i=document.getElementById("hdr-title-discover");i&&(i.textContent=`\u{1F52D} Ontdek Artiesten \xB7 ${a.length} artiesten`);let n=a.reduce((o,r)=>o+r.missingCount,0),d=`<div class="section-title">Gebaseerd op: ${(t||[]).slice(0,3).join(", ")}
    &nbsp;\xB7&nbsp; <span style="color:var(--new)">${n} albums te ontdekken</span></div>
    <div class="discover-grid">`;for(let o=0;o<a.length;o++){let r=a[o],u=Math.round(r.match*100),f=[_(r.country),r.country,r.startYear?`Actief vanaf ${r.startYear}`:null,r.totalAlbums?`${r.totalAlbums} studio-albums`:null].filter(Boolean).join(" \xB7 "),y=I(r.image,120)||r.image,w=y?`<img class="discover-photo" src="${l(y)}" alt="" loading="lazy"
      ...
      <div class="discover-section collapsed" id="disc-${o}">
        ...
        <div class="discover-albums-wrap">`,r.albums?.length){d+='<div class="album-grid">';for(let x of r.albums)d+=K(x,!0,r.name);d+="</div>"}else d+='<div style="font-size:13px;color:var(--muted2);padding:8px 0">Albums nog niet beschikbaar. Vernieuw straks.</div>';d+="</div></div>"}d+="</div>",b(d);let c=document.getElementById("sec-discover-preview");if(c){let o=a.slice(0,8);c.innerHTML=`<div class="collapsed-thumbs">${o.map(r=>r.image?`<div class="collapsed-thumb collapsed-thumb-round">
          <img src="${l(r.image)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
          <span class="collapsed-thumb-ph" style="display:none;background:${g(r.name)}">${m(r.name)}</span>
        </div>`:`<div class="collapsed-thumb collapsed-thumb-round" style="background:${g(r.name)}"><span class="collapsed-thumb-ph">${m(r.name)}</span></div>`).join("")}${a.length>8?`<span class="collapsed-thumbs-more">+${a.length-8}</span>`:""}</div>`}}function ht(){try{let e=localStorage.getItem("ontdek-sections");e&&Object.assign(s.collapsibleSections,JSON.parse(e))}catch{}}function wt(){try{localStorage.setItem("ontdek-sections",JSON.stringify(s.collapsibleSections))}catch{}}function Ke(e,t){e.classList.remove("expanded","collapsed"),e.classList.add(t?"collapsed":"expanded")}function Se(e,t){let a=document.querySelector(`[data-section="${e}"]`);if(!a)return;let i=a.querySelector(".section-toggle-btn");i&&(Ke(i,s.collapsibleSections[t]),i.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),s.collapsibleSections[t]=!s.collapsibleSections[t],wt(),Ke(i,s.collapsibleSections[t]),a.classList.toggle("collapsed")}),s.collapsibleSections[t]&&a.classList.add("collapsed"))}async function W(){ht(),s.currentTab="ontdek",D();let e=s.spotifyEnabled?`
    ...
    <div class="ontdek-layout">
      ...
      <div class="widget-hdr"><span class="widget-title">${l(st(o))}</span></div>
    ...
    <div class="widget-grid" id="widget-grid">${d}</div>`;b(c,()=>{Promise.allSettled([$t(),xt(),Et(),kt(),Lt(),It(),St()]),document.getElementById("dash-customize-btn")?.addEventListener("click",()=>{let o=document.getElementById("dash-customize-panel");o&&(o.style.display=o.style.display==="none"?"":"none")}),document.querySelectorAll(".dash-widget-cb").forEach(o=>{o.addEventListener("change",()=>{let r=[];document.querySelectorAll(".dash-widget-cb").forEach(u=>{u.checked||r.push(u.dataset.widget)}),localStorage.setItem("dashWidgetHidden",JSON.stringify(r)),it()})})})}function V(e,t){let a=document.getElementById("wbody-"+e);a&&(a.innerHTML=`<div class="widget-error">\u26A0 ${l(t||"Niet beschikbaar")}</div>`)}async function $t(){let e=document.getElementById("wbody-nu-luisteren");if(e)try{let[t,a]=await Promise.allSettled([fetch("/api/plex/nowplaying").then(n=>n.json()),p("/api/recent")]),i="";if(t.status==="fulfilled"&&t.value?.playing){let n=t.value;i+=`<div class="w-np-row">
      ...
      </div>`}).join("")}</div>`}catch(t){V("recente-nummers",t.message)}}async function Et(){let e=document.getElementById("wbody-nieuwe-releases");if(e)try{let t=s.lastReleases;if(!t){let n=await p("/api/releases");if(n.status==="building"){e.innerHTML='<div class="empty" style="font-size:12px">Releases worden geladen\u2026</div>';return}t=n.releases||[]}let a=Date.now()-168*3600*1e3,i=t.filter(n=>n.releaseDate&&new Date(n.releaseDate).getTime()>a).sort((n,d)=>(d.artistPlaycount||0)-(n.artistPlaycount||0)).slice(0,3);if(!i.length){e.innerHTML='<div class="empty" style="font-size:12px">Geen releases deze week</div>';return}e.innerHTML=`<div class="w-releases-list">${i.map(n=>`<div class="w-rel-row">
      ...
      </div>`).join("")}</div>`}catch(t){V("nieuwe-releases",t.message)}}async function kt(){let e=document.getElementById("wbody-download-voortgang");if(e){if(!s.tidarrOk){e.innerHTML='<div class="widget-error">\u26A0 Tidarr offline</div>';return}try{let a=((await p("/api/tidarr/queue")).items||s.tidarrQueueItems||[]).filter(i=>i.status!=="finished"&&i.status!=="error");Ee(e,a)}catch{V("download-voortgang","Tidarr niet bereikbaar")}}}async function Lt(){let e=document.getElementById("wbody-vandaag-cijfers");if(e)try{let a=(await p("/api/recent")).recenttracks?.track||[],i=new Date().toDateString(),n=a.filter(r=>r.date?.uts&&new Date(parseInt(r.date.uts)*1e3).toDateString()===i),d=new Set(n.map(r=>r.artist?.["#text"])).size,c={};for(let r of n){let u=r.artist?.["#text"]||"";c[u]=(c[u]||0)+1}let o=Object.entries(c).sort((r,u)=>u[1]-r[1])[0];e.innerHTML=`<div class="w-stats-grid">
    ...
    </div>`}catch(t){V("vandaag-cijfers",t.message)}}async function It(){let e=document.getElementById("wbody-aanbeveling");if(e)try{let t=Math.floor(Date.now()/864e5);if((!ie||at!==t)&&(ie=(await p("/api/recs")).recommendations||[],at=t),!ie.length){e.innerHTML='<div class="empty" style="font-size:12px">Geen aanbevelingen</div>';return}let a=ie[t%ie.length],i=null;try{i=await p(`/api/artist/${encodeURIComponent(a.name)}/info`)}catch{}let n=i?.image?I(i.image,80)||i.image:null,d=(i?.albums||[]).slice(0,3);e.innerHTML=`<div class="w-rec-wrap">
      <div class="w-rec-top">
    ...
    <div class="plib-list">`;for(let[c,o]of n)d+=`
      <div class="plib-artist-block">
      ...
      </div>`;return d+"</div>"}async function ne(){q();try{let e=await p("/api/plex/library");s.plexLibData=e.library||[];let t=document.getElementById("plib-search");if(t&&(t.value=""),!s.plexLibData.length){b('<div class="empty">Plex bibliotheek is leeg of nog niet gesynchroniseerd.<br>Klik \u21BB Sync Plex om te beginnen.</div>');return}b(ge(s.plexLibData,""))}catch(e){C(e.message)}}async function qe(e){q();try{let a=(await p(`/api/topartists?period=${e}`)).topartists?.artist||[];if(!a.length){b('<div class="empty">Geen data.</div>');return}let i=parseInt(a[0]?.playcount||1),n=`<div class="section-title">Top artiesten \xB7 ${Y(e)}</div><div class="artist-grid">`;for(let d=0;d<a.length;d++){let c=a[d],o=Math.round(parseInt(c.playcount)/i*100),r=j(c.image,"large")||j(c.image),u=I(r,120)||r,f=u?`<img src="${u}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
        ...
        </div></div>`}b(n+"</div>"),a.forEach(async(d,c)=>{try{let o=await p(`/api/artist/${encodeURIComponent(d.name)}/info`);if(o.image){let r=document.getElementById(`agp-${c}`);r&&(r.innerHTML=`<img src="${I(o.image,120)||o.image}" alt="" loading="lazy" onerror="this.style.display='none'">`)}}catch{}})}catch(t){C(t.message)}}async function Ae(e){q();try{let a=(await p(`/api/toptracks?period=${e}`)).toptracks?.track||[];if(!a.length){b('<div class="empty">Geen data.</div>');return}let i=parseInt(a[0]?.playcount||1),n=`<div class="section-title">Top nummers \xB7 ${Y(e)}</div><div class="card-list">`;for(let d of a){let c=Math.round(parseInt(d.playcount)/i*100);n+=`<div class="card">${he(d.image)}<div class="card-info">
        ...
        <div class="play-bar"><div class="play-bar-fill"></div></div></div>`}b(n+"</div>")}catch(t){C(t.message)}}async function rt(){q();try{let t=(await p("/api/loved")).lovedtracks?.track||[];if(!t.length){b('<div class="empty">Geen geliefde nummers.</div>');return}let a='<div class="section-title">Geliefde nummers</div><div class="card-list">';for(let i of t){let n=i.date?.uts?Z(parseInt(i.date.uts)):"";a+=`<div class="card">${he(i.image)}<div class="card-info">
        <div class="card-title">${l(i.name)}</div>
        ...
        <div class="play-bar"><div class="play-bar-fill"></div></div></div>`}b(a+"</div>")}catch(e){C(e.message)}}async function Me(){q("Statistieken ophalen...");try{let e=await p("/api/stats");b(`
      ...
      <div class="gaps-block">
      ...
      </details>`),a+="</div>"}b(a)}async function ot(e){s.bibSubTab=e;let t=document.getElementById("bib-sub-content"),a=document.getElementById("bib-subtoolbar");if(!t)return;document.querySelectorAll(".bib-tab").forEach(n=>n.classList.toggle("active",n.dataset.bibtab===e)),a&&(e==="collectie"?(a.innerHTML=`
        ...
        </div>`,document.getElementById("plib-search-bib")?.addEventListener("input",n=>{s.plexLibData&&(t.innerHTML=ge(s.plexLibData,n.target.value))}),document.getElementById("btn-sync-plex-bib")?.addEventListener("click",async()=>{let n=document.getElementById("btn-sync-plex-bib"),d=n.textContent;n.disabled=!0,n.textContent="\u21BB Bezig\u2026";try{await fetch("/api/plex/refresh",{method:"POST"}),await N(),s.plexLibData=null;let c=t;s.sectionContainerEl=c,await ne(),s.sectionContainerEl===c&&(s.sectionContainerEl=null)}catch{}finally{n.disabled=!1,n.textContent=d}})):e==="gaten"?(a.innerHTML=`
        <div class="inline-toolbar" style="margin-bottom:12px">
    ...
    </div>`,T.style.opacity="1",T.style.transform="";let e=document.getElementById("badge-gaps-bib"),t=document.getElementById("badge-gaps");e&&t&&(e.textContent=t.textContent),document.querySelectorAll(".bib-tab").forEach(a=>a.addEventListener("click",()=>ot(a.dataset.bibtab)));{let a=document.getElementById("strip-artists-body");s.sectionContainerEl=a,await qe(s.currentPeriod),s.sectionContainerEl===a&&(s.sectionContainerEl=null)}{let a=document.getElementById("strip-tracks-body");s.sectionContainerEl=a,await Ae(s.currentPeriod),s.sectionContainerEl===a&&(s.sectionContainerEl=null)}await ot(s.bibSubTab),ee(document.getElementById("bib-stats-content"),()=>{let a=document.getElementById("bib-stats-content");return R(a,Me)})}function De(e){let t=document.getElementById("panel-overlay"),a=document.getElementById("panel-content"),i=oe(e),n=()=>{a.innerHTML=`<div style="height:260px;background:var(--surface2)"></div>
      <div class="panel-body"><div class="loading" style="padding:2rem 0"><div class="spinner"></div>Laden...</div></div>`,t.classList.add("open"),document.body.style.overflow="hidden"};document.startViewTransition?document.startViewTransition(n).finished.catch(()=>{}):n(),Promise.allSettled([p(`/api/artist/${encodeURIComponent(e)}/info`),p(`/api/artist/${encodeURIComponent(e)}/similar`)]).then(([d,c])=>{let o=d.status==="fulfilled"?d.value:{},r=c.status==="fulfilled"?c.value.similar||[]:[],u=I(o.image,400)||o.image,f=u?`<img src="${l(u)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"
           onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
         <div class="panel-photo-ph" style="background:${g(e)};display:none">${m(e)}</div>`:`<div class="panel-photo-ph" style="background:${g(e)}">${m(e)}</div>`,y=[o.country?_(o.country)+" "+o.country:null,o.startYear?`Actief vanaf ${o.startYear}`:null,s.plexOk&&o.inPlex!==void 0?o.inPlex?"\u25B6 In Plex":"\u2726 Nieuw voor jou":null].filter(Boolean).join(" \xB7 "),w="";if(o.albums?.length){w='<div class="panel-section">Albums</div><div class="panel-albums">';for(let $ of o.albums){let x=I($.image,48)||$.image,A=x?`<img class="panel-album-img" src="${l(x)}" alt="" loading="lazy" onerror="this.onerror=null;this.remove()">`:'<div class="panel-album-ph">\u266A</div>',M=s.plexOk&&$.inPlex?'<span class="badge plex" style="font-size:9px">\u25B6</span>':"";w+=`<div class="panel-album-row">${A}
