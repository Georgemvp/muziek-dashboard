(()=>{var s={currentTab:"nu",currentMainTab:"nu",bibSubTab:"collectie",sectionContainerEl:null,currentPeriod:"7day",recsFilter:"all",discFilter:"all",gapsSort:"missing",releasesSort:"listening",releasesFilter:"all",plexOk:!1,lastDiscover:null,lastGaps:null,lastReleases:null,plexLibData:null,wishlistMap:new Map,newReleaseIds:new Set,searchTimeout:null,tidalSearchTimeout:null,tidarrOk:!1,tidalView:"search",tidalSearchResults:null,tidarrQueuePoll:null,tidarrSseSource:null,tidarrQueueItems:[],downloadedSet:new Set,spotifyEnabled:!1,activeMood:null,previewAudio:new Audio,previewBtn:null,collapsibleSections:{recs:!1,releases:!1,discover:!1},sectionMutex:Promise.resolve(),dlResolve:null,VALID_QUALITIES:["max","high","normal","low"]};var be=window.matchMedia("(prefers-reduced-motion: reduce)").matches,S=document.getElementById("content");function T(e,t=120){return e?`/api/img?url=${encodeURIComponent(e)}&w=${t}&h=${t}`:null}var z=(e,t="medium")=>{if(!e)return null;let a=e.find(i=>i.size===t);return a&&a["#text"]&&!a["#text"].includes("2a96cbd8b46e442fc41c2b86b821562f")?a["#text"]:null},m=e=>String(e||"?").split(/\s+/).map(t=>t[0]).join("").toUpperCase().slice(0,2),q=e=>parseInt(e).toLocaleString("nl-NL"),l=e=>String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Y=e=>({"7day":"week","1month":"maand","3month":"3 maanden","12month":"jaar",overall:"alles"})[e]||e;function ie(e){let t=Math.floor(Date.now()/1e3)-e;return t<120?"zojuist":t<3600?`${Math.floor(t/60)}m`:t<86400?`${Math.floor(t/3600)}u`:`${Math.floor(t/86400)}d`}function g(e,t=!1){let a=0;for(let r=0;r<e.length;r++)a=a*31+e.charCodeAt(r)&16777215;let i=a%360,n=45+a%31,d=50+(a>>8)%26,c=20+(a>>16)%16,o=15+(a>>10)%11;return t?`radial-gradient(circle, hsl(${i},${n}%,${c}%), hsl(${(i+40)%360},${d}%,${o}%))`:`linear-gradient(135deg, hsl(${i},${n}%,${c}%), hsl(${(i+40)%360},${d}%,${o}%))`}function O(e){return!e||e.length!==2?"":[...e.toUpperCase()].map(t=>String.fromCodePoint(t.charCodeAt(0)+127397)).join("")}function F(e,t=4){return e?.length?`<div class="tags" style="margin-top:5px">${e.slice(0,t).map(a=>`<span class="tag">${l(a)}</span>`).join("")}</div>`:""}function ye(e){return s.plexOk?e?'<span class="badge plex">\u25B6 In Plex</span>':'<span class="badge new">\u2726 Nieuw</span>':""}function N(e,t,a="",i=""){let n=s.wishlistMap.has(`${e}:${t}`);return`<button class="bookmark-btn${n?" saved":""}"
    ...
    <div class="q-list">${t.map(n=>{let d=i[n.status]||"q-pending",c=a[n.status]||n.status||"In wachtrij",o=n.progress?.current&&n.progress?.total?Math.round(n.progress.current/n.progress.total*100):null,r=o!==null?`<div class="q-bar"><div class="q-bar-fill" style="width:${o}%"></div></div><div class="q-pct">${o}%</div>`:"";return`<div class="q-row">
      ...
      <div class="q-list">${t.map(i=>{let n=i.queued_at?new Date(i.queued_at).toLocaleDateString("nl-NL",{day:"numeric",month:"short",year:"numeric"}):"",d=a[i.quality]||i.quality||"";return`<div class="q-row">
        ...
        </div>`}).join("")}</div>`,document.getElementById("dl-history-clear")?.addEventListener("click",async()=>{if(confirm("Wis de volledige download-geschiedenis?")){await fetch("/api/downloads",{method:"DELETE"}).catch(()=>{});for(let i of t)await fetch(`/api/downloads/${i.id}`,{method:"DELETE"}).catch(()=>{});s.downloadedSet.clear(),Ue()}})}catch(t){e.innerHTML=`<div class="error-box">\u26A0\uFE0F ${l(t.message)}</div>`}}}function oe(e){s.tidalView=e,document.querySelectorAll("[data-tidal-view]").forEach(t=>t.classList.toggle("sel-def",t.dataset.tidalView===e)),e==="search"?$e(document.getElementById("tidal-search")?.value||""):e==="queue"?Ve():e==="history"&&Ue()}function re(){if(s.tidarrSseSource)return;let e=new EventSource("/api/tidarr/stream");s.tidarrSseSource=e,e.onmessage=t=>{try{s.tidarrQueueItems=JSON.parse(t.data)||[]}catch{s.tidarrQueueItems=[]}let a=s.tidarrQueueItems.filter(n=>n.status!=="finished"&&n.status!=="error"),i=[document.getElementById("badge-tidarr-queue"),document.getElementById("badge-tidarr-queue-inline")];for(let n of i)n&&(a.length>0?(n.textContent=a.length,n.style.display=""):n.style.display="none");ot(s.tidarrQueueItems),s.currentTab==="tidal"&&s.tidalView==="queue"&&Ve(),s.currentTab==="nu"&&typeof dw_downloadVoortgang!=="undefined"&&dw_downloadVoortgang(),document.getElementById("queue-popover")?.classList.contains("open")&&_e()},e.onerror=()=>{e.close(),s.tidarrSseSource=null,setTimeout(re,1e4)}}function lt(){re()}function Ge(){let e=document.getElementById("tidarr-iframe"),t=document.getElementById("tidarr-ui-wrap"),a=document.getElementById("content");t.style.display="flex",a.style.display="none",e.dataset.loaded||(e.src=e.dataset.src,e.dataset.loaded="1")}function D(){document.getElementById("tidarr-ui-wrap").style.display="none",document.getElementById("content").style.display=""}function ot(e){let t=document.getElementById("queue-fab"),a=document.getElementById("fab-queue-badge");if(!t)return;let i=(e||[]).filter(n=>n.status!=="finished"&&n.status!=="error");e&&e.length>0?(t.style.display="",i.length>0?(a.textContent=i.length,a.style.display=""):a.style.display="none"):(t.style.display="none",document.getElementById("queue-popover")?.classList.remove("open"))}function _e(){let e=document.getElementById("queue-popover-list");if(!e)return;let t=s.tidarrQueueItems;if(!t.length){e.innerHTML='<div class="qpop-empty">Queue is leeg</div>';return}let a={queue_download:"In wachtrij",queue_processing:"Verwerken",download:"Downloaden\u2026",processing:"Verwerken\u2026",finished:"Klaar \u2713",error:"Fout"},i={queue_download:"q-pending",queue_processing:"q-pending",download:"q-active",processing:"q-active",finished:"q-done",error:"q-error"};e.innerHTML=t.map(n=>{let d=i[n.status]||"q-pending",c=a[n.status]||n.status||"In wachtrij",o=n.progress?.current&&n.progress?.total?Math.round(n.progress.current/n.progress.total*100):null,r=o!==null?`<div class="q-bar" style="margin-top:4px"><div class="q-bar-fill" style="width:${o}%"></div></div>`:"";return`<div class="qpop-row">
      <div class="qpop-title">${l(n.title||"(onbekend)")}</div>
      ${n.artist?`<div class="qpop-artist">${l(n.artist)}</div>`:""}
      <span class="q-status ${d}">${l(c)}</span>
      ${r}
    </div>`}).join("")}function rt(){let e=document.getElementById("queue-popover");if(!e)return;e.classList.toggle("open")&&_e()}function xe(){document.getElementById("queue-popover")?.classList.remove("open")}function Oe(e){return(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g,"")}function Qe(e,t){let a=Oe(e),i=Oe(t);return!a||!i?!0:a===i||a.includes(i)||i.includes(a)}function dt(e,t,a,i){return new Promise(n=>{s.dlResolve=n;let d=document.getElementById("dl-confirm-modal"),c=document.getElementById("dl-confirm-cards");document.getElementById("dl-confirm-wanted").textContent=`"${a}"${t?" \u2013 "+t:""}`,c.innerHTML=e.map((o,r)=>{let u=!Qe(o.artist,t),v=o.image?`<img class="dlc-img" src="${l(o.image)}" alt="" loading="lazy"
             onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
...
Probeer het handmatig via de \u{1F30A} Tidal-tab.`),a&&(a.disabled=!1,a.textContent="\u2B07");return}let{candidates:d}=await n.json();if(!d?.length){alert(`Niet gevonden op TIDAL: "${t}"${e?" van "+e:""}`),a&&(a.disabled=!1,a.textContent="\u2B07");return}let c=d[0];if(e&&!Qe(c.artist,e)){a&&(a.disabled=!1,a.textContent="\u2B07");let{chosen:o}=await dt(d,e,t,a);if(!o)return;a&&(a.disabled=!0,a.textContent="\u2026"),await Ne(o,e,t,a)}else await Ne(c,e,t,a)}catch(i){alert("Downloaden mislukt: "+i.message),a&&(a.disabled=!1,a.textContent="\u2B07")}}async function ke(){b('<div id="tidal-content"><div class="empty">Begin met typen om te zoeken op Tidal.</div></div>'),await we(),await le(),oe(s.tidalView),lt()}function Ye(){s.currentTab="tidal",D(),document.getElementById("tb-tidal")?.classList.add("visible"),ke()}document.getElementById("dl-confirm-cancel")?.addEventListener("click",()=>{Ee()});document.getElementById("dl-confirm-modal")?.addEventListener("click",e=>{e.target===document.getElementById("dl-confirm-modal")&&Ee()});document.getElementById("queue-fab")?.addEventListener("click",rt);document.getElementById("qpop-close")?.addEventListener("click",e=>{e.stopPropagation(),xe()});document.getElementById("qpop-goto-tidal")?.addEventListener("click",()=>{xe(),document.querySelector('.tab[data-tab="downloads"]')?.click(),setTimeout(()=>oe("queue"),150)});document.addEventListener("click",e=>{let t=document.getElementById("queue-popover"),a=document.getElementById("queue-fab");t?.classList.contains("open")&&!t.contains(e.target)&&!a?.contains(e.target)&&xe()},!0);document.getElementById("btn-tidarr-reload")?.addEventListener("click",()=>{let e=document.getElementById("tidarr-iframe");e.src=e.dataset.src});async function Ze(){try{let e=await f("/api/spotify/status");s.spotifyEnabled=!!e.enabled;let t=document.getElementById("tb-mood");s.spotifyEnabled&&s.currentTab==="recs"?(t.style.display="",t.classList.add("visible")):s.spotifyEnabled&&(t.style.display="")}catch{s.spotifyEnabled=!1}}function ct(e,t){let a=e.image?`<img src="${l(e.image)}" alt="" loading="lazy"
    ...
    <div class="spotify-card">
      ...
      ${s.plexOk?` &nbsp;\xB7&nbsp; <span style="color:var(--new)">${n} nieuw</span> \xB7 <span style="color:var(--plex)">${d} in Plex</span>`:""}
        ...
        <div class="trackrec-list">`;for(let u of i){let v=u.playcount>0?`<span class="trackrec-plays">${q(u.playcount)}\xD7</span>`:"",y=u.url?`<a class="trackrec-link" href="${l(u.url)}" target="_blank" rel="noopener">Last.fm \u2197</a>`:"";o+=`
          ...
          </div>`}o+="</div>"}b(o,()=>{s.activeMood&&ce(s.activeMood)}),ue();let r=document.getElementById("sec-recs-preview");if(r){let u=t.slice(0,8);r.innerHTML=`<div class="collapsed-thumbs">${u.map((v,y)=>`<div class="collapsed-thumb collapsed-thumb-round" id="recs-thumb-${y}" style="background:${g(v.name)}">
          <span class="collapsed-thumb-ph">${m(v.name)}</span>
        </div>`).join("")}${t.length>8?`<span class="collapsed-thumbs-more">+${t.length-8}</span>`:""}</div>`,u.forEach(async(v,y)=>{try{let w=await f(`/api/artist/${encodeURIComponent(v.name)}/info`),E=document.getElementById(`recs-thumb-${y}`);E&&w.image&&(E.innerHTML=`<img src="${l(T(w.image,48)||w.image)}" alt="" loading="lazy" onerror="this.remove()">`)}catch{}})}t.forEach(async(u,v)=>{try{let y=await f(`/api/artist/${encodeURIComponent(u.name)}/info`),w=document.getElementById(`rph-${v}`);w&&y.image&&(w.innerHTML=`<img src="${T(y.image,120)||y.image}" alt="" loading="lazy"
          onerror="this.parentElement.innerHTML='<div class=\\'rec-photo-ph\\' style=\\'background:${g(u.name)}\\'>${m(u.name)}</div>'">`);let E=document.getElementById(`rtags-${v}`);E&&(E.innerHTML=F(y.tags,3)+'<div style="height:6px"></div>');let $=document.getElementById(`ralb-${v}`);if($){let x=(y.albums||[]).slice(0,4);if(x.length){let A='<div class="rec-albums-label">Bekende albums</div><div class="rec-albums-list">';for(let P of x){let p=P.image?`<img class="rec-album-img" src="${T(P.image,48)||P.image}" alt="" loading="lazy">`:'<div class="rec-album-ph">\u266A</div>',h=s.plexOk&&P.inPlex?'<span class="rec-album-plex">\u25B6</span>':"";A+=`<div class="rec-album-row">${p}<span class="rec-album-name">${l(P.name)}</span>${h}${V(u.name,P.name,P.inPlex)}</div>`}$.innerHTML=A+"</div>"}else $.innerHTML=""}}catch{let y=document.getElementById(`ralb-${v}`);y&&(y.innerHTML="")}})}catch(e){B(e.message)}}function ue(){document.querySelectorAll(".rec-card[data-inplex]").forEach(e=>{let t=e.dataset.inplex==="true",a=!0;s.recsFilter==="new"&&(a=!t),s.recsFilter==="plex"&&(a=t),e.classList.toggle("hidden",!a)})}function Xe(e){let t=document.getElementById("badge-releases");t&&(e>0?(t.textContent=e,t.style.display=""):t.style.display="none")}function ut(e){if(!e)return"";let t=new Date(e),i=Math.floor((new Date-t)/864e5);return i===0?"vandaag":i===1?"gisteren":i<7?`${i} dagen geleden`:t.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})}async function U(){C();try{let e=await f("/api/releases");if(e.status==="building"){b(`<div class="loading"><div class="spinner"></div>
        <div>${l(e.message)}</div>
    ...
    <div class="releases-grid">`;for(let o of t){let r=s.newReleaseIds.has(`${o.artist}::${o.album}`),u=o.image?`<img class="rel-img" src="${l(o.image)}" alt="" loading="lazy"
      ...
      <div class="rel-card${r?" rel-card-new":""}">
        ...
        <div class="rel-info">
          ...
          ${w}
          <div class="rel-footer">${E}${$}${V(o.artist,o.album,o.inPlex)}</div>
        </div>
    ...
    <div class="discover-grid">`;for(let o=0;o<a.length;o++){let r=a[o],u=Math.round(r.match*100),v=[O(r.country),r.country,r.startYear?`Actief vanaf ${r.startYear}`:null,r.totalAlbums?`${r.totalAlbums} studio-albums`:null].filter(Boolean).join(" \xB7 "),y=T(r.image,120)||r.image,w=y?`<img class="discover-photo" src="${l(y)}" alt="" loading="lazy"
      ...
      <div class="discover-section collapsed" id="disc-${o}">
        ...
        <div class="discover-albums-wrap">`,r.albums?.length){d+='<div class="album-grid">';for(let x of r.albums)d+=Z(x,!0,r.name);d+="</div>"}else d+='<div style="font-size:13px;color:var(--muted2);padding:8px 0">Albums nog niet beschikbaar. Vernieuw straks.</div>';d+="</div></div>"}d+="</div>",b(d);let c=document.getElementById("sec-discover-preview");if(c){let o=a.slice(0,8);c.innerHTML=`<div class="collapsed-thumbs">${o.map(r=>r.image?`<div class="collapsed-thumb collapsed-thumb-round">
          <img src="${l(r.image)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
          <span class="collapsed-thumb-ph" style="display:none;background:${g(r.name)}">${m(r.name)}</span>
        </div>`:`<div class="collapsed-thumb collapsed-thumb-round" style="background:${g(r.name)}"><span class="collapsed-thumb-ph">${m(r.name)}</span></div>`).join("")}${a.length>8?`<span class="collapsed-thumbs-more">+${a.length-8}</span>`:""}</div>`}}function pt(){try{let e=localStorage.getItem("ontdek-sections");e&&Object.assign(s.collapsibleSections,JSON.parse(e))}catch{}}function mt(){try{localStorage.setItem("ontdek-sections",JSON.stringify(s.collapsibleSections))}catch{}}function Je(e,t){e.classList.remove("expanded","collapsed"),e.classList.add(t?"collapsed":"expanded")}function Le(e,t){let a=document.querySelector(`[data-section="${e}"]`);if(!a)return;let i=a.querySelector(".section-toggle-btn");i&&(Je(i,s.collapsibleSections[t]),i.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),s.collapsibleSections[t]=!s.collapsibleSections[t],mt(),Je(i,s.collapsibleSections[t]),a.classList.toggle("collapsed")}),s.collapsibleSections[t]&&a.classList.add("collapsed"))}async function Q(){pt(),s.currentTab="ontdek",D();let e=s.spotifyEnabled?`
    ...
    <div class="plib-list">`;for(let[c,o]of n)d+=`
      ...
      <div class="gaps-block">
        ...
        <div class="gaps-album-grid">`;for(let r of i.missingAlbums)a+=Z(r,!1,i.name);a+="</div>",i.allAlbums?.filter(r=>r.inPlex).length>0&&(a+=`<details style="margin-top:12px">
      ...
      </details>`),a+="</div>"}b(a)}async function et(e){s.bibSubTab=e;let t=document.getElementById("bib-sub-content"),a=document.getElementById("bib-subtoolbar");if(!t)return;document.querySelectorAll(".bib-tab").forEach(n=>n.classList.toggle("active",n.dataset.bibtab===e)),a&&(e==="collectie"?(a.innerHTML=`
    ...
    <div class="bib-layout">
      ...
      <div class="panel-body"><div class="loading" style="padding:2rem 0"><div class="spinner"></div>Laden...</div></div>`,t.classList.add("open"),document.body.style.overflow="hidden"};document.startViewTransition?document.startViewTransition(n).finished.catch(()=>{}):n(),Promise.allSettled([f(`/api/artist/${encodeURIComponent(e)}/info`),f(`/api/artist/${encodeURIComponent(e)}/similar`)]).then(([d,c])=>{let o=d.status==="fulfilled"?d.value:{},r=c.status==="fulfilled"?c.value.similar||[]:[],u=T(o.image,400)||o.image,v=u?`<img src="${l(u)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"
           onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
         <div class="panel-photo-ph" style="background:${g(e)};display:none">${m(e)}</div>`:`<div class="panel-photo-ph" style="background:${g(e)}">${m(e)}</div>`,y=[o.country?O(o.country)+" "+o.country:null,o.startYear?`Actief vanaf ${o.startYear}`:null,s.plexOk&&o.inPlex!==void 0?o.inPlex?"\u25B6 In Plex":"\u2726 Nieuw voor jou":null].filter(Boolean).join(" \xB7 "),w="";if(o.albums?.length){w='<div class="panel-section">Albums</div><div class="panel-albums">';for(let $ of o.albums){let x=T($.image,48)||$.image,A=x?`<img class="panel-album-img" src="${l(x)}" alt="" loading="lazy" onerror="this.onerror=null;this.remove()">`:'<div class="panel-album-ph">\u266A</div>',P=s.plexOk&&$.inPlex?'<span class="badge plex" style="font-size:9px">\u25B6</span>':"";w+=`<div class="panel-album-row">${A}
          <span class="panel-album-name">${l($.name)}</span>${P}${V(e,$.name,$.inPlex)}</div>`}w+="</div>"}let E="";if(r.length){E='<div class="panel-section">Vergelijkbare artiesten</div><div class="panel-similar">';for(let $ of r)E+=`<button class="panel-similar-chip artist-link" data-artist="${l($.name)}">${l($.name)}</button>`;E+="</div>"}a.innerHTML=`
...
async function loadDashboard(){
  ...
  const html=`<div class="dashboard-topbar">
  ...
  <div class="dash-customize-panel" id="dash-customize-panel" style="display:none">
    ...
    <div class="dash-widget-checkboxes">
      ${DEFAULT_WIDGETS.map(w=>`<label class="dash-widget-label"><input type="checkbox" class="dash-widget-cb" data-widget="${l(w)}"${hiddenWidgets.includes(w)?'':' checked'}> ${l(wLabel(w))}</label>`).join('')}
    </div>
  </div>
  <div class="widget-grid" id="widget-grid">
    ${visible.map(w=>`<div class="widget-card" id="widget-${l(w)}" data-widget="${l(w)}"><div class="widget-hdr"><span class="widget-title">${l(wLabel(w))}</span></div><div class="widget-body" id="wbody-${l(w)}"><div class="loading" style="padding:12px 0"><div class="spinner"></div></div></div></div>`).join('')}
  </div>`;

  b(html,()=>{
    Promise.allSettled([dw_nuLuisteren(),dw_recenteNummers(),dw_nieuweReleases(),dw_downloadVoortgang(),dw_vandaagCijfers(),dw_aanbeveling(),dw_collectieStats()]);
    document.getElementById('dash-customize-btn')?.addEventListener('click',()=>{
      const p=document.getElementById('dash-customize-panel');
      if(p)p.style.display=p.style.display==='none'?'':'none';
    });
    document.querySelectorAll('.dash-widget-cb').forEach(cb=>{
      cb.addEventListener('change',()=>{
        const hidden=[];
        document.querySelectorAll('.dash-widget-cb').forEach(c=>{if(!c.checked)hidden.push(c.dataset.widget);});
...
function dw_err(id,msg){const el=document.getElementById('wbody-'+id);if(el)el.innerHTML=`<div class="widget-error">⚠ ${l(msg||'Niet beschikbaar')}</div>`;}
...
async function dw_nuLuisteren(){
  ...
  try{
    ...
    if(plexRes.status==='fulfilled'&&plexRes.value?.playing){
      ...
      html+=`<div class="w-np-row"><div class="w-np-dot plex"></div><div class="w-np-info"><div class="w-np-title">${l(p.track)}</div><div class="w-np-sub">${l(p.artist)}${p.album?' · '+l(p.album):''}</div><span class="badge plex" style="font-size:10px">▶ Plex</span></div></div>`;
    }
    if(lfmRes.status==='fulfilled'){
      ...
      if(np){
        ...
        const img=z(np.image,'medium');
        html+=`<div class="w-np-row"><div class="w-np-dot lfm"></div>${img?`<img class="w-np-img" src="${l(img)}" alt="" loading="lazy">`:''}
          <div class="w-np-info"><div class="w-np-title">${l(np.name)}</div><div class="w-np-sub artist-link" data-artist="${l(artist)}">${l(artist)}</div>
          <span class="badge" style="background:var(--red);color:#fff;font-size:10px">● Last.fm</span></div></div>`;
      }
    }
    el.innerHTML=html||'<div class="empty" style="font-size:12px;padding:8px 0">Niets aan het afspelen</div>';
  }catch(e){dw_err('nu-luisteren',e.message);}
}
...
async function dw_recenteNummers(){
  ...
  if(!el)return;
  try{
    const data=await f('/api/recent');
    const tracks=(data.recenttracks?.track||[]).filter(t=>!t['@attr']?.nowplaying).slice(0,8);
    ...
    el.innerHTML=`<div class="w-track-list">${tracks.map(t=>{
      ...
      const ago=t.date?.uts?ie(parseInt(t.date.uts)):'';
      return`<div class="w-track-row"><div class="w-track-info"><div class="w-track-title">${l(t.name)}</div><div class="w-track-artist artist-link" data-artist="${l(artist)}">${l(artist)}</div></div><span class="w-track-ago">${ago}</span></div>`;
    }).join('')}</div>`;
  }catch(e){dw_err('recente-nummers',e.message);}
}
...
async function dw_nieuweReleases(){
  ...
  if(!el)return;
  try{
    let releases=s.lastReleases;
    if(!releases){
      const data=await f('/api/releases');
      if(data.status==='building'){el.innerHTML='<div class="empty" style="font-size:12px">Releases worden geladen…</div>';return;}
    ...
    el.innerHTML=`<div class="w-releases-list">${recent.map(r=>{
      const img=r.image?`<img class="w-rel-img" src="${l(r.image)}" alt="" loading="lazy" onerror="this.style.display='none'">`:`<div class="w-rel-ph" style="background:${g(r.album)}">${m(r.album)}</div>`;
      return`<div class="w-rel-row"><div class="w-rel-cover">${img}</div><div class="w-rel-info"><div class="w-rel-title">${l(r.album)}</div><div class="w-rel-artist artist-link" data-artist="${l(r.artist)}">${l(r.artist)}</div></div>${V(r.artist,r.album,r.inPlex)}</div>`;
    }).join('')}</div>`;
  }catch(e){dw_err('nieuwe-releases',e.message);}
}

async function dw_downloadVoortgang(){
  const el=document.getElementById('wbody-download-voortgang');
  if(!el)return;
  if(!s.tidarrOk){el.innerHTML='<div class="widget-error">⚠ Tidarr offline</div>';return;}
  try{
    const data=await f('/api/tidarr/queue');
    const items=(data.items||s.tidarrQueueItems||[]).filter(i=>i.status!=='finished'&&i.status!=='error');
    if(!items.length){el.innerHTML='<div class="empty" style="font-size:12px">Geen actieve downloads</div>';return;}
    const statusLabel={queue_download:'In wachtrij',queue_processing:'Verwerken',download:'Downloaden…',processing:'Verwerken…'};
    el.innerHTML=`<div class="w-queue-list">${items.slice(0,5).map(i=>{
      const pct=i.progress?.current&&i.progress?.total?Math.round(i.progress.current/i.progress.total*100):null;
      return`<div class="w-q-row"><div class="w-q-info"><div class="w-q-title">${l(i.title||'(onbekend)')}</div>${i.artist?`<div class="w-q-artist">${l(i.artist)}</div>`:''}`+
        (pct!==null?`<div class="q-bar" style="margin-top:4px"><div class="q-bar-fill" style="width:${pct}%"></div></div><div class="q-pct" style="font-size:10px;color:var(--muted2);margin-top:2px">${pct}%</div>`:`<span class="q-status q-pending" style="margin-top:4px;display:inline-block">${l(statusLabel[i.status]||i.status)}</span>`)+
        `</div></div>`;
    }).join('')}${items.length>5?`<div style="font-size:11px;color:var(--muted2);margin-top:6px">+${items.length-5} meer</div>`:''}</div>`;
  }catch(e){dw_err('download-voortgang','Tidarr niet bereikbaar');}
}
...
async function dw_vandaagCijfers(){
  ...
  try{
    ...
    const todayTracks=tracks.filter(t=>t.date?.uts&&new Date(parseInt(t.date.uts)*1000).toDateString()===today);
    const uniqueArtists=new Set(todayTracks.map(t=>t.artist?.['#text'])).size;
    const artistCount={};
    for(const t of todayTracks){const a=t.artist?.['#text']||'';artistCount[a]=(artistCount[a]||0)+1;}
    const topEntry=Object.entries(artistCount).sort((a,b)=>b[1]-a[1])[0];
    el.innerHTML=`<div class="w-stats-grid">
      <div class="w-stat-block"><div class="w-stat-val">${todayTracks.length}</div><div class="w-stat-lbl">scrobbles</div></div>
      <div class="w-stat-block"><div class="w-stat-val">${uniqueArtists}</div><div class="w-stat-lbl">artiesten</div></div>
      ${topEntry?`<div class="w-stat-block w-stat-wide"><div class="w-stat-val" style="font-size:13px;line-height:1.3">${l(topEntry[0])}</div><div class="w-stat-lbl">meest gespeeld (${topEntry[1]}×)</div></div>`:''}
    </div>`;
  }catch(e){dw_err('vandaag-cijfers',e.message);}
}
...
async function dw_aanbeveling(){
  ...
  try{
    ...
    if(!_dashRecsCache||_dashRecsCacheDay!==today){
      const data=await f('/api/recs');
      _dashRecsCache=data.recommendations||[];
      _dashRecsCacheDay=today;
    }
    const recs=_dashRecsCache;
    if(!recs.length){el.innerHTML='<div class="empty" style="font-size:12px">Geen aanbevelingen</div>';return;}
    const pick=recs[today%recs.length];
    let info=null;
    try{info=await f(`/api/artist/${encodeURIComponent(pick.name)}/info`);}catch{}
    const img=info?.image?T(info.image,80)||info.image:null;
    const albs=(info?.albums||[]).slice(0,3);
    el.innerHTML=`<div class="w-rec-wrap"><div class="w-rec-top">
      ${img?`<img class="w-rec-img" src="${l(img)}" alt="" loading="lazy">`:`<div class="w-rec-ph" style="background:${g(pick.name)}">${m(pick.name)}</div>`}
      <div class="w-rec-info">
        <div class="w-rec-name artist-link" data-artist="${l(pick.name)}">${l(pick.name)}</div>
        <div class="w-rec-reason">Vergelijkbaar met ${l(pick.reason)}</div>
        ${ye(pick.inPlex)}${N('artist',pick.name,'',info?.image||'')}
      </div></div>
      ${albs.length?`<div class="w-rec-albums">${albs.map(a=>`<span class="w-rec-album">${l(a.name)}</span>`).join('')}</div>`:''}
    </div>`;
  }catch(e){dw_err('aanbeveling',e.message);}
}
...
async function dw_collectieStats(){
  ...
  if(!el)return;
  try{
    const data=await f('/api/plex/status');
    if(!data.connected){el.innerHTML='<div class="empty" style="font-size:12px">Plex offline</div>';return;}
    let totalMissing=0;
    if(s.lastGaps?.artists)totalMissing=s.lastGaps.artists.reduce((a,g)=>a+(g.missingCount||0),0);
    el.innerHTML=`<div class="w-stats-grid">
      <div class="w-stat-block"><div class="w-stat-val">${q(data.artists||0)}</div><div class="w-stat-lbl">artiesten</div></div>
      <div class="w-stat-block"><div class="w-stat-val">${q(data.albums||0)}</div><div class="w-stat-lbl">albums</div></div>
      ${totalMissing?`<div class="w-stat-block"><div class="w-stat-val">${totalMissing}</div><div class="w-stat-lbl">ontbreekt</div></div>`:''}
    </div>`;
  }catch(e){dw_err('collectie-stats',e.message);}
}
