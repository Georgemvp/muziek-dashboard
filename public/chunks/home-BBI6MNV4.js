      ...
      <div class="home-wylbt-card">
        ...
        <div id="home-wylbt-releases-list">
          ${dt(a)}
            ...
            </div>`}).join("");t.innerHTML=n,t.querySelectorAll(".home-artist-circle-item").forEach(r=>{r.addEventListener("click",()=>{let c=r.dataset.artist;c&&$("artist-detail",{name:c})})})}}}catch(a){console.warn("Recent artists herlaad mislukt:",a)}}async function zt(){if(G.user?.name)return G.user.name;try{let e=await p("/api/core/user");return e?.user?.name||e?.name||null}catch{return null}}function mt(e){if(!e)return{};let a={...e};return e.artists!==void 0&&e.artistCount===void 0&&(a.artistCount=e.artists),e.albums!==void 0&&e.albumCount===void 0&&(a.albumCount=e.albums),e.tracks!==void 0&&e.trackCount===void 0&&(a.trackCount=e.tracks),a.artistCount!==void 0?a:e.library?mt(e.library):e.stats?e.stats:a}async function Qt(){let e=document.getElementById("content");if(!e)return;e.innerHTML=`
    <div class="home-page" aria-busy="true" aria-label="Laden\u2026">
      ${Z()}
      ${F(6,1)}
      ${F(4,2)}
    </div>`;let[a,s,t,i,n,r,c]=await Promise.all([zt().catch(()=>null),p("/api/plex/status").catch(()=>null),p("/api/plex/stats?period=7day").catch(()=>null),p("/api/wishlist").catch(()=>null),p("/api/core/releases").catch(()=>null),p("/api/core/user").catch(()=>null),p("/api/core/loved").catch(()=>null)]),d,y,m;t&&t.source==="plex"?(d=it(t.topArtists),y=nt(t.topTracks),m={recenttracks:{track:U(t.recentTracks)}}):[d,y,m]=await Promise.all([p("/api/core/top/artists?period=7day").catch(()=>null),p("/api/core/top/tracks?period=7day").catch(()=>null),p("/api/core/recent?limit=200").catch(()=>null)]);let g={ok:r&&!r._stale,user:r?.user?.name||null},h=mt(s),v=m?.recenttracks?.track||[],k=i?.wishlist||i||[],b=n,L=c?.lovedtracks?.track||[],B=Ct(d),ut=await Lt(d).catch(()=>""),ht=await It(d).catch(()=>""),vt=await Tt().catch(()=>""),j=v,z=null;if(t?.source==="plex"&&t?.dailyPlays?.some(l=>l.minutes>0||l.count>0))z=t.dailyPlays;else try{let l=await p("/api/activity?period=1month");l?.dailyPlays?.some(u=>u.minutes>0||u.count>0)&&(z=l.dailyPlays,l.recentTracks?.length&&(j=U(l.recentTracks)))}catch{try{j=(await p("/api/core/recent?limit=200"))?.recenttracks?.track||v}catch{}}e.innerHTML=`
    <div class="home-page">
