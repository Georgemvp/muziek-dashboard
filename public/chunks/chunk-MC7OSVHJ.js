import{a as it,f as C}from"./chunk-5VRNHLFG.js";import{A as M,B as _,E as g,G as X,X as tt,a as b,b as I,c as q,f as S,g as G,h as y,i as R,j as l,k as j,l as N,m as f,n as V,o as Y,q as J,r as Q,t as O,u as F,v as m,w,x as L}from"./chunk-RRZ2Q5W2.js";async function rt(){try{let t=await g("/api/wishlist");b.wishlistMap.clear();for(let i of t)b.wishlistMap.set(`${i.type}:${i.name}`,i.id);D()}catch{}}function D(){let t=document.getElementById("badge-wishlist");t&&(t.textContent=b.wishlistMap.size||"0")}async function Bt(t,i,e,s){let a=`${t}:${i}`;if(b.wishlistMap.has(a)){try{await I(`/api/wishlist/${b.wishlistMap.get(a)}`,{method:"DELETE"})}catch(n){if(n.name!=="AbortError")throw n}return b.wishlistMap.delete(a),D(),!1}else{let o=await(await I("/api/wishlist",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:t,name:i,artist:e,image:s})})).json();return b.wishlistMap.set(a,o.id),D(),!0}}async function et(){L(),await rt();try{let t=await g("/api/wishlist");if(!t.length){m('<div class="empty">Je lijst is leeg.<br>Voeg artiesten toe via het \u{1F516} icoon in Ontdek en Collectiegaten.</div>');return}let i=`<div class="section-title">${t.length} opgeslagen</div><div class="wishlist-grid">`;for(let e of t){let s=e.image?`<img src="${l(e.image)}" alt="" loading="lazy"
