import{a as w}from"./chunk-6N34G4LY.js";import{a as G,c as U}from"./chunk-2UCV5F4T.js";import{A as v,a as B,e as M,i as l,k as _}from"./chunk-LCSFBDUL.js";var T=["#1a237e","#283593","#3949ab","#5c6bc0","#7986cb","#9fa8da"],C=null;function V(e){return!e||!Array.isArray(e)?{topartists:{artist:[]}}:{topartists:{artist:e.map(s=>({name:s.name,playcount:String(s.playcount||0),image:[null,null,{"#text":s.thumb||""},{"#text":s.thumb||""}],topTag:s.genre||null}))}}}function W(e){return!e||!Array.isArray(e)?{toptracks:{track:[]}}:{toptracks:{track:e.map(s=>({name:s.title,playcount:String(s.playcount||0),artist:{name:s.artist,"#text":s.artist},album:{"#text":s.album,name:s.album},image:[null,null,{"#text":s.thumb||""}]}))}}}function N(e){return!e||!Array.isArray(e)?[]:e.map(s=>({name:s.title,artist:{"#text":s.artist},album:{"#text":s.album},image:[null,null,{"#text":s.thumb||""}],date:{uts:String(s.viewedAt)}}))}function I(e){return e==null||isNaN(e)?"\u2014":Number(e).toLocaleString("nl-NL")}function q(e){if(!e)return"";let s=new Date(e),a=Date.now()-s.getTime(),t=Math.floor(a/864e5);return t<1?"Vandaag":t===1?"Gisteren":t<7?`${t}d geleden`:t<31?`${Math.floor(t/7)}w geleden`:`${Math.floor(t/30)}mo geleden`}function x(e,s=120){return e?M(e,s):null}function rt(e,s,a){let t=e||"Muzikant",i=s?.artists??s?.artistCount??"\u2026",n=s?.albums??s?.albumCount??"\u2026",r=s?.tracks??s?.trackCount??"\u2026",o=s?.composers??s?.composerCount??"\u2014",d='<svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>',p='<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3"/><line x1="12" y1="3" x2="12" y2="9"/></svg>',y='<svg viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>',g='<svg viewBox="0 0 24 24"><path d="M9 12h6M9 8h6M9 16h4"/><rect x="3" y="4" width="18" height="16" rx="2"/></svg>',m=a?.ok?`<span style="width: 8px; height: 8px; background: #4caf50; border-radius: 50%; display: inline-block; margin-left: 8px; title='Last.fm connected'"></span>`:`<span style="width: 8px; height: 8px; background: #f44336; border-radius: 50%; display: inline-block; margin-left: 8px;" title='Last.fm unavailable'></span>`;return`
    ...
    </div>`}function ot(e){if(!e||e<1)return"0m";if(e<60)return`${Math.round(e)}m`;let s=Math.floor(e/60),a=Math.round(e%60);return a>0?`${s}h ${a}m`:`${s}h`}function K(e,s){let a={};if(s&&Array.isArray(s))for(let m of s)m.date!=null&&(a[m.date]=m.minutes||(m.count?m.count*3.5:0));else for(let m of e||[]){let u=m.date?.uts;if(!u)continue;let b=new Date(parseInt(u,10)*1e3).toISOString().slice(0,10);a[b]=(a[b]||0)+3.5}let t=new Date,i=t.getDay(),n=new Date(t);n.setDate(t.getDate()-(i+6)%7),n.setHours(0,0,0,0);let r=[];for(let m=0;m<4;m++){let u=new Date(n);u.setDate(n.getDate()-m*7);let $=[];for(let b=0;b<7;b++){let L=new Date(u);L.setDate(u.getDate()+b);let R=L.toISOString().slice(0,10);$.push({key:R,minutes:a[R]||0})}r.push({days:$,totalMinutes:$.reduce((b,L)=>b+L.minutes,0)})}let o=Math.max(...r.map(m=>m.totalMinutes),1),d=Math.max(...r.flatMap(m=>m.days.map(u=>u.minutes)),1);function p(m){if(!m)return"width:6px;height:6px;background:#e0e0e0;";let u=m/d;return u<.25?"width:10px;height:10px;background:var(--accent);opacity:0.4;":u<.6?"width:16px;height:16px;background:var(--accent);opacity:0.7;":"width:24px;height:24px;background:var(--accent);opacity:1;"}let y=r.map(m=>{let u=m.totalMinutes>0?Math.round(m.totalMinutes/o*100):0,$=m.days.map(b=>`<div class="activity-dot-cell"><div class="activity-dot" style="${p(b.minutes)}" title="${b.key}: ${Math.round(b.minutes)}min"></div></div>`).join("");return`
      <div class="activity-bar-wrap">
        <div class="activity-bar" style="width:${u}%"></div>
        <span class="activity-bar-label">${ot(m.totalMinutes)}</span>
      </div>
      ${$}`}).join("");return`
    <div class="home-wylbt-card activity-matrix-card">
    ...
    </div>`}function S(e,s="played"){return e?.length?e.slice(0,8).map(t=>{let i=t.image?.[2]?.["#text"]||t.image?.[1]?.["#text"]||t.image?.[3]?.["#text"]||"",n=i?x(i,140):null,r=n?`<img src="${l(n)}" alt="${l(t.name)}" loading="lazy">`:'<div class="home-recent-cover-ph">\u266A</div>',o="";if(s==="added"&&t.addedAt)o=q(t.addedAt);else if(s==="played"&&t.date?.uts){let d=new Date(parseInt(t.date.uts,10)*1e3);o=q(d.toISOString())}return`
      ...
      </div>`}).join(""):'<div style="color:rgba(255,255,255,0.5);font-size:13px;padding:12px 0">Geen recente activiteit</div>'}function ct(e){return`
    ...
    </div>`}function dt(e){let s=(e||[]).slice(0,8);return s.length?`
    <div class="home-recent-banner">
      <div class="home-recent-header">
    ...
    </div>`:""}function mt(e){let s=(e||[]).slice(0,4);return s.length?`
    ...
    </div>`}function vt(e){let s=(e?.topartists?.artist||[]).slice(0,5);return s.length?`
    <div class="home-section-header">
    ...
    <div class="home-recent-artists-wrap">
      ...
      </div>`:""}catch(s){return console.warn("Recommended artists render mislukt:",s),""}}async function yt(){try{let e=await v("/api/plex/playlists"),s=(e?.playlists||e||[]).slice(0,5);return s.length?`
      ...
      </div>`:""}catch(e){return console.warn("Playlists render mislukt:",e),""}}function gt(e){let a=(e?.topartists?.artist||[]).slice(0,2).map((t,i)=>{let n=t.image?.[3]?.["#text"]||t.image?.[2]?.["#text"]||"",r=n?M(n,400):"";return`
    ...
    <div class="releases-main-card">
    ...
    </div>`,m=o.slice(0,2).map((u,$)=>{let b=x(u.image||u.thumb,160);return`
      <div class="releases-small-card">
        ...
        <div class="releases-small-info">
          ...
          <div class="releases-plex-badge" id="plex-badge-${$+1}" style="display:none" title="Beschikbaar in Plex">Q</div>
        </div>
      </div>`}).join("");return`
    <div class="releases-preview">
      ${g}
      <div class="releases-stack">${m}</div>
    </div>`}function wt(e,s="albums"){return`
    ${`
    <div class="home-section-header">
    ...
    <div id="releases-body">
      ...
      </div>`}).join("")}function kt(e,s){return`
      ...
      <div class="home-wylbt-card">
        ...
        <div id="home-wylbt-releases-list">
          ${Z(s)}
            ...
            </div>`}).join("");t.innerHTML=n,t.querySelectorAll(".home-artist-circle-item").forEach(r=>{r.addEventListener("click",()=>{let o=r.dataset.artist;o&&w("artist-detail",{name:o})})})}}}catch(s){console.warn("Recent artists herlaad mislukt:",s)}}async function It(){if(B.user?.name)return B.user.name;try{let e=await v("/api/user");return e?.user?.name||e?.name||null}catch{return null}}function tt(e){if(!e)return{};let s={...e};return e.artists!==void 0&&e.artistCount===void 0&&(s.artistCount=e.artists),e.albums!==void 0&&e.albumCount===void 0&&(s.albumCount=e.albums),e.tracks!==void 0&&e.trackCount===void 0&&(s.trackCount=e.tracks),s.artistCount!==void 0?s:e.library?tt(e.library):e.stats?e.stats:s}async function Ht(){let e=document.getElementById("content");if(!e)return;e.innerHTML=`
    <div class="home-page" aria-busy="true" aria-label="Laden\u2026">
      ${U()}
      ${G(6,1)}
      ${G(4,2)}
    </div>`;let[s,a,t,i,n,r,o]=await Promise.all([It().catch(()=>null),v("/api/plex/status").catch(()=>null),v("/api/plex/stats?period=7day").catch(()=>null),v("/api/wishlist").catch(()=>null),v("/api/releases").catch(()=>null),v("/api/user").catch(()=>null),v("/api/loved").catch(()=>null)]),d,p,y;t&&t.source==="plex"?(d=V(t.topArtists),p=W(t.topTracks),y={recenttracks:{track:N(t.recentTracks)}}):[d,p,y]=await Promise.all([v("/api/topartists?period=7day").catch(()=>null),v("/api/toptracks?period=7day").catch(()=>null),v("/api/recent?limit=200").catch(()=>null)]);let g={ok:r&&!r._stale,user:r?.user?.name||null},m=tt(a),u=y?.recenttracks?.track||[],$=i?.wishlist||i||[],b=n,L=o?.lovedtracks?.track||[],R=bt(d),et=await ut(d).catch(()=>""),st=await ht(d).catch(()=>""),at=await yt().catch(()=>""),H=u,D=null;if(t?.source==="plex"&&t?.dailyPlays?.some(c=>c.minutes>0||c.count>0))D=t.dailyPlays;else try{let c=await v("/api/activity?period=1month");c?.dailyPlays?.some(h=>h.minutes>0||h.count>0)&&(D=c.dailyPlays,c.recentTracks?.length&&(H=N(c.recentTracks)))}catch{try{H=(await v("/api/recent?limit=200"))?.recenttracks?.track||u}catch{}}e.innerHTML=`
    <div class="home-page">
      ...
      <!-- 3d. Your Playlists -->
      <div id="home-playlists-container">${at}</div>

      <!-- 4. Daily Mixes -->
      <div>${gt(d)}</div>

      <!-- 4b. Genres for you -->
      <div id="home-genres-section">${ft(R)}</div>

      <!-- 5. New Releases -->
      <div id="home-releases-section">${wt(b)}</div>
