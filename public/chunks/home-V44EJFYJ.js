import{a as T}from"./chunk-6YLQFTAJ.js";import{a as A}from"./chunk-N2OPSRFD.js";import"./chunk-C6ZPWJ3O.js";import{A as v,a as C,e as M,i as l,k as _}from"./chunk-GHKYA3ZE.js";var S=["#1a237e","#283593","#3949ab","#5c6bc0","#7986cb","#9fa8da"],H=null;function Y(e){return!e||!Array.isArray(e)?{topartists:{artist:[]}}:{topartists:{artist:e.map(s=>({name:s.name,playcount:String(s.playcount||0),image:[null,null,{"#text":s.thumb||""},{"#text":s.thumb||""}],topTag:s.genre||null}))}}}function V(e){return!e||!Array.isArray(e)?{toptracks:{track:[]}}:{toptracks:{track:e.map(s=>({name:s.title,playcount:String(s.playcount||0),artist:{name:s.artist,"#text":s.artist},album:{"#text":s.album,name:s.album},image:[null,null,{"#text":s.thumb||""}]}))}}}function N(e){return!e||!Array.isArray(e)?[]:e.map(s=>({name:s.title,artist:{"#text":s.artist},album:{"#text":s.album},image:[null,null,{"#text":s.thumb||""}],date:{uts:String(s.viewedAt)}}))}function I(e){return e==null||isNaN(e)?"\u2014":Number(e).toLocaleString("nl-NL")}function U(e){if(!e)return"";let s=new Date(e),a=Date.now()-s.getTime(),t=Math.floor(a/864e5);return t<1?"Vandaag":t===1?"Gisteren":t<7?`${t}d geleden`:t<31?`${Math.floor(t/7)}w geleden`:`${Math.floor(t/30)}mo geleden`}function x(e,s=120){return e?M(e,s):null}function nt(e,s,a){let t=e||"Muzikant",i=s?.artists??s?.artistCount??"\u2026",n=s?.albums??s?.albumCount??"\u2026",r=s?.tracks??s?.trackCount??"\u2026",o=s?.composers??s?.composerCount??"\u2014",d='<svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>',p='<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3"/><line x1="12" y1="3" x2="12" y2="9"/></svg>',y='<svg viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>',g='<svg viewBox="0 0 24 24"><path d="M9 12h6M9 8h6M9 16h4"/><rect x="3" y="4" width="18" height="16" rx="2"/></svg>',m=a?.ok?`<span style="width: 8px; height: 8px; background: #4caf50; border-radius: 50%; display: inline-block; margin-left: 8px; title='Last.fm connected'"></span>`:`<span style="width: 8px; height: 8px; background: #f44336; border-radius: 50%; display: inline-block; margin-left: 8px;" title='Last.fm unavailable'></span>`;return`
    ...
    </div>`}function lt(e){if(!e||e<1)return"0m";if(e<60)return`${Math.round(e)}m`;let s=Math.floor(e/60),a=Math.round(e%60);return a>0?`${s}h ${a}m`:`${s}h`}function W(e,s){let a={};if(s&&Array.isArray(s))for(let m of s)m.date!=null&&(a[m.date]=m.minutes||(m.count?m.count*3.5:0));else for(let m of e||[]){let u=m.date?.uts;if(!u)continue;let b=new Date(parseInt(u,10)*1e3).toISOString().slice(0,10);a[b]=(a[b]||0)+3.5}let t=new Date,i=t.getDay(),n=new Date(t);n.setDate(t.getDate()-(i+6)%7),n.setHours(0,0,0,0);let r=[];for(let m=0;m<4;m++){let u=new Date(n);u.setDate(n.getDate()-m*7);let w=[];for(let b=0;b<7;b++){let E=new Date(u);E.setDate(u.getDate()+b);let P=E.toISOString().slice(0,10);w.push({key:P,minutes:a[P]||0})}r.push({days:w,totalMinutes:w.reduce((b,E)=>b+E.minutes,0)})}let o=Math.max(...r.map(m=>m.totalMinutes),1),d=Math.max(...r.flatMap(m=>m.days.map(u=>u.minutes)),1);function p(m){if(!m)return"width:6px;height:6px;background:#e0e0e0;";let u=m/d;return u<.25?"width:10px;height:10px;background:var(--accent);opacity:0.4;":u<.6?"width:16px;height:16px;background:var(--accent);opacity:0.7;":"width:24px;height:24px;background:var(--accent);opacity:1;"}let y=r.map(m=>{let u=m.totalMinutes>0?Math.round(m.totalMinutes/o*100):0,w=m.days.map(b=>`<div class="activity-dot-cell"><div class="activity-dot" style="${p(b.minutes)}" title="${b.key}: ${Math.round(b.minutes)}min"></div></div>`).join("");return`
      <div class="activity-bar-wrap">
        <div class="activity-bar" style="width:${u}%"></div>
        <span class="activity-bar-label">${lt(m.totalMinutes)}</span>
      </div>
      ${w}`}).join("");return`
    <div class="home-wylbt-card activity-matrix-card">
    ...
    </div>`}function R(e,s="played"){return e?.length?e.slice(0,8).map(t=>{let i=t.image?.[2]?.["#text"]||t.image?.[1]?.["#text"]||t.image?.[3]?.["#text"]||"",n=i?x(i,140):null,r=n?`<img src="${l(n)}" alt="${l(t.name)}" loading="lazy">`:'<div class="home-recent-cover-ph">\u266A</div>',o="";if(s==="added"&&t.addedAt)o=U(t.addedAt);else if(s==="played"&&t.date?.uts){let d=new Date(parseInt(t.date.uts,10)*1e3);o=U(d.toISOString())}return`
      ...
      </div>`}).join(""):'<div style="color:rgba(255,255,255,0.5);font-size:13px;padding:12px 0">Geen recente activiteit</div>'}function ot(e){return`
    ...
    </div>`}function ct(e){let s=(e||[]).slice(0,8);return s.length?`
    <div class="home-recent-banner">
      <div class="home-recent-header">
      ...
      <div class="home-recent-row">
        <div class="home-recent-covers-wrap">
          <div class="home-recent-covers" id="home-loved-covers">
            ${s.map(t=>{let i=t.image?.[2]?.["#text"]||t.image?.[1]?.["#text"]||t.image?.[3]?.["#text"]||"",n=i?x(i,140):null;return`
      <div class="home-recent-cover">
    ...
    </div>`:""}function dt(e){let s=(e||[]).slice(0,4);return s.length?`
    ...
    <div class="home-listen-later-grid">${s.map(t=>{let i=x(t.image,96);return`
    ...
    </div>`}function ut(e){let s=(e?.topartists?.artist||[]).slice(0,5);return s.length?`
    <div class="home-section-header">
    ...
    <div class="home-recent-artists-wrap">
      <div class="home-recent-artists" id="home-recent-artists">${s.map(t=>{let i=x(t.image?.[3]?.["#text"]||t.image?.[2]?.["#text"],200),n=i?`<img class="home-artist-circle-img" src="${l(i)}" alt="${l(t.name)}" loading="lazy">`:'<div class="home-artist-circle-ph">\u266A</div>';return`
      ...
      </div>`:""}catch(s){return console.warn("Recommended artists render mislukt:",s),""}}async function ht(){try{let e=await v("/api/plex/playlists"),s=(e?.playlists||e||[]).slice(0,5);return s.length?`
      ...
      </div>`:""}catch(e){return console.warn("Playlists render mislukt:",e),""}}function yt(e){let a=(e?.topartists?.artist||[]).slice(0,2).map((t,i)=>{let n=t.image?.[3]?.["#text"]||t.image?.[2]?.["#text"]||"",r=n?M(n,400):"";return`
    ...
    </div>`}function K(e,s){let a=e?.releases||(Array.isArray(e)?e:[]),t=a.filter(u=>(u.type||"album").toLowerCase()!=="single"),i=a.filter(u=>(u.type||"").toLowerCase()==="single"),n=(s==="singles"?i:t).slice(0,3);if(!n.length)return'<div style="padding:32px;text-align:center;color:var(--text-muted);font-size:14px">Geen releases gevonden.</div>';let[r,...o]=n,d=x(r.image||r.thumb,400),p=d?`<img src="${l(d)}" alt="${l(r.title||r.album||"")}" loading="lazy">`:'<div class="releases-main-ph">\u266B</div>',y=r.description||r.bio||"",g=`
    ...
    </div>`,m=o.slice(0,2).map((u,w)=>{let b=x(u.image||u.thumb,160);return`
    ...
    </div>`}function ft(e,s="albums"){return`
    ${`
    <div class="home-section-header">
    ...
    <div id="releases-body">
      ${K(e,s)}
    </div>`}async function q(e,s){let a=e?.releases||(Array.isArray(e)?e:[]),t=a.filter(o=>(o.type||"album").toLowerCase()!=="single"),i=a.filter(o=>(o.type||"").toLowerCase()==="single"),n=(s==="singles"?i:t).slice(0,3);if(!n.length)return;let r=n.map(o=>({artist:o.artist||"",album:o.title||o.album||""}));try{let o=await fetch("/api/plex/check-batch",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:r})});if(!o.ok)throw new Error(`HTTP ${o.status}`);let p=(await o.json()).results||{};r.forEach((y,g)=>{let m=`${y.artist}||${y.album}`,u=document.getElementById(`plex-badge-${g}`);u&&p[m]&&(u.style.display="inline-flex")})}catch(o){console.warn("Plex check-batch aanroep mislukt:",o)}}function wt(e){let s=document.getElementById("home-donut-chart");!s||!window.Chart||(H&&(H.destroy(),H=null),H=new window.Chart(s,{type:"doughnut",data:{labels:e.map(a=>a.name),datasets:[{data:e.map(a=>a.count),backgroundColor:e.map(a=>a.color),borderWidth:0,hoverOffset:4}]},options:{cutout:"65%",plugins:{legend:{display:!1},tooltip:{callbacks:{label:a=>` ${a.label}: ${I(a.parsed)} plays`}}},animation:{duration:400}}}))}async function X(e){let s=document.getElementById("home-genres-legend");try{let a=null;try{let t=await v(`/api/plex/stats?period=${e}`);if(t&&t.source==="plex"&&t.genres&&Array.isArray(t.genres)&&t.genres.length>0){let i=t.genres.reduce((n,r)=>n+r.count,0)||1;a=t.genres.map((n,r)=>({name:n.name,count:n.count,pct:Math.round(n.count/i*100),color:S[r%S.length]})).slice(0,6)}}catch{}if(!a){let i=((await v(`/api/top/artists?period=${e}`))?.topartists?.artist||[]).slice(0,8),n={};for(let y of i){let g=y.topTag||"Other",m=parseInt(y.playcount,10)||0;n[g]=(n[g]||0)+m}let r=Object.entries(n).sort((y,g)=>g[1]-y[1]),o=r.slice(0,6),d=r.slice(6).reduce((y,[,g])=>y+g,0);if(d>0){let y=o.findIndex(([g])=>g==="Other");y>=0?o[y][1]+=d:o.push(["Other",d])}let p=o.reduce((y,[,g])=>y+g,0)||1;a=o.map(([y,g],m)=>({name:y,count:g,pct:Math.round(g/p*100),color:S[m%S.length]}))}s&&(s.innerHTML=a.map(t=>`
      ...
      </div>`}).join("")}function xt(e){let s={};for(let a of e?.toptracks?.track||[]){let t=a.album?.["#text"]||a.album?.name||null,i=a.artist?.name||a.artist?.["#text"]||a.artist||"";if(!t)continue;let n=`${t}|||${i}`;s[n]||(s[n]={album:t,artist:i,playcount:0,image:a.image}),s[n].playcount+=parseInt(a.playcount,10)||0}return Object.values(s).sort((a,t)=>t.playcount-a.playcount).slice(0,4)}function Q(e){let s=xt(e);if(!s.length)return'<div style="color:var(--text-muted);font-size:13px">Geen data</div>';let a=s[0]?.playcount||1;return s.map(t=>{let i=Math.round(t.playcount/a*100),n=x(t.image?.[2]?.["#text"]||t.image?.[1]?.["#text"],72);return`
      ...
      </div>`}).join("")}function $t(e,s){return`
      ...
      <div class="home-wylbt-card">
        ...
        <div id="home-wylbt-releases-list">
          ${Q(s)}
    ...
    </div>`}function kt(){let e=document.getElementById("home-recent-covers"),s=document.getElementById("home-recent-prev"),a=document.getElementById("home-recent-next");if(!e||!s||!a)return;let t=0,i=160;function n(){let r=Math.max(0,e.scrollWidth-e.parentElement.offsetWidth);t=Math.max(0,Math.min(t,r)),e.style.transform=`translateX(-${t}px)`,s.disabled=t<=0,a.disabled=t>=r}s.addEventListener("click",()=>{t=Math.max(0,t-i),n()}),a.addEventListener("click",()=>{t+=i,n()}),n()}async function Et(e){let s,a;try{let n=await v(`/api/plex/stats?period=${e}`);n&&n.source==="plex"?(s=Y(n.topArtists),a=V(n.topTracks)):[s,a]=await Promise.all([v(`/api/topartists?period=${e}`).catch(()=>null),v(`/api/toptracks?period=${e}`).catch(()=>null)])}catch{[s,a]=await Promise.all([v(`/api/topartists?period=${e}`).catch(()=>null),v(`/api/toptracks?period=${e}`).catch(()=>null)])}let t=document.getElementById("home-wylbt-artists-list");t&&(t.innerHTML=J(s));let i=document.getElementById("home-wylbt-releases-list");i&&(i.innerHTML=Q(a)),t?.querySelectorAll("[data-artist]").forEach(n=>{n.addEventListener("click",()=>T(n.dataset.artist))}),await X(e)}async function Lt(e){localStorage.setItem("homePeriod",e),document.querySelectorAll(".home-period-pill").forEach(s=>{s.classList.toggle("active",s.dataset.period===e)});try{let s=null,a=[],t=await v(`/api/plex/stats?period=${e}`);if(t?.dailyPlays?.some(n=>n.minutes>0||n.count>0))s=t.dailyPlays,a=t.recentTracks||[];else{let n=await v(`/api/activity?period=${e}`);n?.dailyPlays?.some(r=>r.minutes>0||r.count>0)&&(s=n.dailyPlays,a=n.recentTracks||[])}let i=document.querySelector(".activity-matrix-card");if(i){let n=a.length?N(a):[];i.outerHTML=W(n,s)}}catch(s){console.warn("Activity matrix herlaad mislukt:",s)}if(e==="today")try{let s=new Date;s.setHours(0,0,0,0);let a=Math.floor(s.getTime()/1e3),i=((await v("/api/plex/stats?period=today"))?.recentTracks||[]).filter(r=>parseInt(r.date?.uts||0,10)>=a),n=document.getElementById("home-recent-covers");n&&(n.innerHTML=R(i))}catch(s){console.warn("Recent activity herlaad mislukt:",s)}else try{let a=(await v(`/api/plex/stats?period=${e}`))?.recentTracks||[],t=document.getElementById("home-recent-covers");t&&(t.innerHTML=R(a))}catch(s){console.warn("Recent activity herlaad mislukt:",s)}await Et(e);try{let a=(await v(`/api/plex/stats?period=${e}`))?.topArtists||[],t=document.getElementById("home-recent-artists");if(t){let i=a.slice(0,5);if(i.length){let n=i.map(r=>{let o=r.thumb||r.image?.[3]?.["#text"]||r.image?.[2]?.["#text"],d=x(o,200),p=d?`<img class="home-artist-circle-img" src="${l(d)}" alt="${l(r.name)}" loading="lazy">`:'<div class="home-artist-circle-ph">\u266A</div>';return`
            ...
            </div>`}).join("");t.innerHTML=n,t.querySelectorAll(".home-artist-circle-item").forEach(r=>{r.addEventListener("click",()=>{let o=r.dataset.artist;o&&T(o)})})}}}catch(s){console.warn("Recent artists herlaad mislukt:",s)}}async function At(){if(C.user?.name)return C.user.name;try{let e=await v("/api/user");return e?.user?.name||e?.name||null}catch{return null}}function Z(e){if(!e)return{};let s={...e};return e.artists!==void 0&&e.artistCount===void 0&&(s.artistCount=e.artists),e.albums!==void 0&&e.albumCount===void 0&&(s.albumCount=e.albums),e.tracks!==void 0&&e.trackCount===void 0&&(s.trackCount=e.tracks),s.artistCount!==void 0?s:e.library?Z(e.library):e.stats?e.stats:s}async function Ct(){let e=document.getElementById("content");if(!e)return;e.innerHTML=`
    <div class="home-page">
    ...
    </div>`;let[s,a,t,i,n,r,o]=await Promise.all([At().catch(()=>null),v("/api/plex/status").catch(()=>null),v("/api/plex/stats?period=7day").catch(()=>null),v("/api/wishlist").catch(()=>null),v("/api/releases").catch(()=>null),v("/api/user").catch(()=>null),v("/api/loved").catch(()=>null)]),d,p,y;t&&t.source==="plex"?(d=Y(t.topArtists),p=V(t.topTracks),y={recenttracks:{track:N(t.recentTracks)}}):[d,p,y]=await Promise.all([v("/api/topartists?period=7day").catch(()=>null),v("/api/toptracks?period=7day").catch(()=>null),v("/api/recent?limit=200").catch(()=>null)]);let g={ok:r&&!r._stale,user:r?.user?.name||null},m=Z(a),u=y?.recenttracks?.track||[],w=i?.wishlist||i||[],b=n,E=o?.lovedtracks?.track||[],P=pt(d),tt=await mt(d).catch(()=>""),et=await vt(d).catch(()=>""),st=await ht().catch(()=>""),D=u,O=null;if(t?.source==="plex"&&t?.dailyPlays?.some(c=>c.minutes>0||c.count>0))O=t.dailyPlays;else try{let c=await v("/api/activity?period=1month");c?.dailyPlays?.some(h=>h.minutes>0||h.count>0)&&(O=c.dailyPlays,c.recentTracks?.length&&(D=N(c.recentTracks)))}catch{try{D=(await v("/api/recent?limit=200"))?.recenttracks?.track||u}catch{}}e.innerHTML=`
    <div class="home-page">
      ...
      <!-- 3d. Your Playlists -->
      <div id="home-playlists-container">${st}</div>

      <!-- 4. Daily Mixes -->
      <div>${yt(d)}</div>

      <!-- 4b. Genres for you -->
      <div id="home-genres-section">${bt(P)}</div>

      <!-- 5. New Releases -->
      <div id="home-releases-section">${ft(b)}</div>
    ...
    </div>`;let at=localStorage.getItem("homePeriod")||"7day";e.querySelectorAll(".home-period-pill").forEach(c=>{c.classList.toggle("active",c.dataset.period===at)}),e.querySelectorAll(".home-period-pill").forEach(c=>{c.addEventListener("click",async()=>{let h=c.dataset.period;h&&await Lt(h)})}),kt(),X("7day"),e.querySelectorAll("[data-switch]").forEach(c=>{c.addEventListener("click",()=>A(c.dataset.switch))}),e.querySelectorAll("#home-wylbt-artists-list [data-artist]").forEach(c=>{c.addEventListener("click",()=>T(c.dataset.artist))}),document.getElementById("home-recent-more")?.addEventListener("click",()=>{A("albums")}),document.getElementById("home-loved-more")?.addEventListener("click",()=>{A("listen-later")}),e.querySelectorAll(".genre-card").forEach(c=>{c.addEventListener("click",()=>A("ontdek")),c.addEventListener("keydown",h=>{(h.key==="Enter"||h.key===" ")&&A("ontdek")})});let B="albums";e.querySelectorAll("[data-releases-tab]").forEach(c=>{c.addEventListener("click",()=>{B=c.dataset.releasesTab,e.querySelectorAll("[data-releases-tab]").forEach(f=>f.classList.toggle("active",f===c));let h=document.getElementById("releases-body");h&&(h.innerHTML=K(b,B),q(b,B))})});let F="played";e.querySelectorAll("[data-recent-tab]").forEach(c=>{c.addEventListener("click",async()=>{e.querySelectorAll("[data-recent-tab]").forEach(f=>f.classList.toggle("active",f===c)),F=c.dataset.recentTab;let h=document.getElementById("home-recent-covers");if(h)if(F==="added")try{let k=((await v("/api/plex/library?sort=addedAt:desc&limit=8"))?.library||[]).map(L=>({name:L.album,artist:{"#text":L.artist},image:L.thumb?[null,null,{"#text":L.thumb}]:[null,null,{"#text":""}],addedAt:new Date(L.addedAt*1e3).toISOString()}));h.innerHTML=R(k,"added")}catch(f){console.warn("Failed to load added items:",f),h.innerHTML='<div style="color:rgba(255,255,255,0.5);font-size:13px;padding:12px 0">Fout bij laden van recent toegevoegde albums</div>'}else h.innerHTML=R(u,"played")})}),e.querySelectorAll(".home-artist-circle-item").forEach(c=>{c.addEventListener("click",()=>{let h=c.dataset.artist;h&&T(h)})});let z=document.getElementById("featured-play-btn");z&&z.addEventListener("click",async()=>{let c=z.dataset.artist;if(c)try{let f=(await v(`/api/artist/${encodeURIComponent(c)}`))?.ratingKey;if(!f){console.warn("Geen ratingKey voor artiest:",c);return}let{playOnZone:k}=await import("./plexRemote-4B76C6KE.js");await k(f,"music")}catch(h){console.error("Featured artist play mislukt:",h)}});let j=document.getElementById("home-recent-artists");j&&(document.getElementById("home-artists-prev")?.addEventListener("click",()=>{j.scrollBy({left:-256,behavior:"smooth"})}),document.getElementById("home-artists-next")?.addEventListener("click",()=>{j.scrollBy({left:256,behavior:"smooth"})}));let G=document.getElementById("home-recommended-artists");G&&(document.getElementById("home-rec-artists-prev")?.addEventListener("click",()=>{G.scrollBy({left:-320,behavior:"smooth"})}),document.getElementById("home-rec-artists-next")?.addEventListener("click",()=>{G.scrollBy({left:320,behavior:"smooth"})})),e.querySelectorAll(".home-rec-artist").forEach(c=>{c.addEventListener("click",()=>{let h=c.dataset.artist;h&&T(h)})});let $=document.getElementById("home-playlists");if($){let h=document.getElementById("home-playlists-prev"),f=document.getElementById("home-playlists-next"),k=()=>{let L=$.scrollLeft<=0,it=$.scrollLeft>=$.scrollWidth-$.clientWidth-10;h?.toggleAttribute("disabled",L),f?.toggleAttribute("disabled",it)};h?.addEventListener("click",()=>{$.scrollBy({left:-400,behavior:"smooth"}),setTimeout(k,400)}),f?.addEventListener("click",()=>{$.scrollBy({left:400,behavior:"smooth"}),setTimeout(k,400)}),$.addEventListener("scroll",k),k()}e.querySelectorAll(".home-playlist-card").forEach(c=>{c.addEventListener("click",()=>{let h=c.dataset.playlistId,f=c.dataset.playlistName;h&&(window.lastSelectedPlaylistId!==void 0&&(C.selectedPlaylist={id:h,name:f}),A("albums"))})}),gt(d),q(b,B)}export{Ct as loadHome};
