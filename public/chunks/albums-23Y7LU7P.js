import{i as y,n as g}from"./chunk-C6ZPWJ3O.js";import{A as x,e as z,g as E,h as _,i as r,k}from"./chunk-GHKYA3ZE.js";var h=null,v=null,m="",b="artist",d="grid",L=null,C=0,I=210,T=3;function f(){return document.getElementById("content")}function H(){let t=window.innerWidth;return t>=1600?8:t>=1300?7:t>=1050?6:t>=850?5:t>=650?4:t>=480?3:2}function D(t){if(!t)return"-";let e=Math.floor(t/60),s=t%60;return`${e}:${s.toString().padStart(2,"0")}`}async function K(t){L=t;let e=f();if(!e)return;C=e.scrollTop||0,await P()}function B(){L=null;let t=f();if(t){let e=S(h||[]);t.innerHTML=`
      <div style="display: flex; gap: 8px; flex: 1;">
        <div style="flex: 1;" id="albums-container"></div>
        <div id="albums-az-rail" class="albums-az-rail"></div>
      </div>
    `;let s=t.querySelector("#albums-container");s&&R(s,e).then(()=>{setTimeout(()=>{t.scrollTop=C},0)})}}async function P(){let t=f();if(!t||!L)return;let e=L,s=e.thumb?z(e.thumb,320):null,l=y(),a=[];try{let n=await x(`/api/plex/album/${e.ratingKey}/tracks`);n&&Array.isArray(n.tracks)&&(a=n.tracks)}catch(n){console.error("Error loading album tracks:",n)}let i=a.length?`<div class="album-detail-tracks">
         ...
         <div class="album-detail-tracklist">
           ${a.map((n,o)=>`
             <div class="album-track" data-rating-key="${r(n.ratingKey)}">
               <div class="album-track-num">${o+1}</div>
               <div class="album-track-title">${r(n.title)}</div>
               <div class="album-track-duration">${D(n.duration)}</div>
    ...
    <div class="album-detail-view">
      ...
      <div class="album-detail-hero">
        <div class="album-detail-cover-wrapper">
          ${s?`<img src="${r(s)}" alt="${r(e.album)}" class="album-detail-cover">`:`<div class="album-detail-cover-ph" style="background:${k(e.album)}">${E(e.album)}</div>`}
        </div>
        <div class="album-detail-info">
          <h1>${r(e.album)}</h1>
          <button class="album-detail-artist-link" data-artist="${r(e.artist)}">${r(e.artist)}</button>
  ...
  `,q(e,a)}function q(t,e){let s=f();if(!s)return;s.querySelectorAll(".album-track").forEach(n=>{n.addEventListener("mouseenter",()=>{let o=n.querySelector(".album-track-num"),u=n.querySelector(".album-track-play");o&&(o.style.opacity="0"),u&&(u.style.opacity="1")}),n.addEventListener("mouseleave",()=>{let o=n.querySelector(".album-track-num"),u=n.querySelector(".album-track-play");o&&(o.style.opacity="1"),u&&(u.style.opacity="0")})});let a=s.querySelector(".album-detail-play-btn"),i=s.querySelector(".album-detail-plex-btn");a&&a.addEventListener("click",async()=>{y()&&await g(t.ratingKey,"music")}),i&&i.addEventListener("click",async()=>{y()&&await g(t.ratingKey,"music")})}function S(t){let e=t;if(m.trim()){let l=m.toLowerCase();e=e.filter(a=>a.artist.toLowerCase().includes(l)||a.album.toLowerCase().includes(l))}let s=[...e];switch(b){case"artist-za":s.sort((l,a)=>{let i=a.artist.localeCompare(l.artist);return i!==0?i:a.album.localeCompare(l.album)});break;case"album":s.sort((l,a)=>l.album.localeCompare(a.album));break;case"album-za":s.sort((l,a)=>a.album.localeCompare(l.album));break;case"recent":s.sort((l,a)=>(a.addedAt||0)-(l.addedAt||0));break;case"artist":default:s.sort((l,a)=>{let i=l.artist.localeCompare(a.artist);return i!==0?i:l.album.localeCompare(a.album)});break}return s}async function Z(){if(h)return h;try{let t=await x("/api/plex/library/all");if(!t||!t.library)return console.warn("Albums API response is null/undefined:",t),[];if(!Array.isArray(t.library))return console.warn("Library is not an array:",t.library),[];if(!t.library.length)return[];let e=t.library.map(([l,a,i,n,o])=>({artist:l||"",album:a||"",ratingKey:i||"",thumb:n||"",addedAt:o||0,playcount:0})),s={};try{let l=await x("/api/top/albums?period=overall");l?.topalbums?.album&&l.topalbums.album.forEach(a=>{let i=`${(a.artist||"").toLowerCase()}|${(a.name||"").toLowerCase()}`;s[i]={playcount:a.playcount||0}})}catch(l){console.warn("Error loading Last.fm albums:",l)}return h=e.map(l=>{let a=`${l.artist.toLowerCase()}|${l.album.toLowerCase()}`,i=s[a];return{...l,playcount:i?.playcount||0}}),h}catch(t){return console.error("Error loading albums:",t),[]}}function F(t){let e=new Map;for(let s of t){let l=(s.artist[0]||"#").toUpperCase(),a=/[A-Z]/.test(l)?l:"#";e.has(a)||e.set(a,[]),e.get(a).push(s)}return e}function G(t){let e=t.thumb?z(t.thumb,240):null,s=e?`<img src="${r(e)}" alt="${r(t.album)} by ${r(t.artist)}" loading="lazy" decoding="async"
         onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
       <div class="albums-cover-ph" style="display:none;background:${k(t.album)}">${E(t.album)}</div>`:`<div class="albums-cover-ph" style="background:${k(t.album)}">${E(t.album)}</div>`,l=t.playcount&&t.playcount>0?`<span class="album-playcount-badge" style="position:absolute;top:8px;right:8px;background:var(--accent);color:white;font-size:11px;padding:4px 8px;border-radius:4px;font-weight:600;">${t.playcount}</span>`:"";return`<div class="albums-album"
    data-rating-key="${r(t.ratingKey)}"
    data-album="${r(t.album)}"
    data-artist="${r(t.artist)}"
    data-thumb="${r(t.thumb||"")}">
    <div class="albums-cover">
      ${s}
      ${l}
      <div class="albums-play-overlay"><button class="albums-play-btn" title="Play">\u25B6</button></div>
    </div>
    <div class="albums-album-title" title="${r(t.album)}">${r(t.album)}</div>
    <button class="albums-artist-btn" title="${r(t.artist)}">${r(t.artist)}</button>
  ...
  `,U()}function U(){document.getElementById("albums-search")?.addEventListener("input",t=>{m=t.target.value,w()}),document.getElementById("albums-btn-grid")?.addEventListener("click",()=>{d!=="grid"&&(d="grid",$(),w())}),document.getElementById("albums-btn-list")?.addEventListener("click",()=>{d!=="list"&&(d="list",$(),w())}),document.getElementById("albums-sort-select")?.addEventListener("change",t=>{b=t.target.value,$(),w()})}function w(){let t=document.getElementById("albums-container");if(t){let e=S(h||[]);R(t,e)}}async function R(t,e){v&&(v.destroy(),v=null);let s=e||S(h||[]);if(O(s.length),!s.length){t.innerHTML=`
      ...
      </div>`;return}v=new A(t,s),V(v)}async function Q(t){let e=t.target.closest(".albums-album");if(e&&!t.target.closest(".albums-play-btn")){let i={ratingKey:e.dataset.ratingKey,album:e.dataset.album,artist:e.dataset.artist,thumb:e.dataset.thumb};return await K(i),!0}let s=t.target.closest(".albums-play-btn");if(s){let i=s.closest(".albums-album");if(i){t.stopPropagation();let n=i.dataset.ratingKey;y()&&await g(n,"music")}return!0}if(t.target.closest(".album-detail-back"))return B(),!0;let l=t.target.closest(".album-detail-artist-link");if(l){m=l.dataset.artist,B();let i=document.getElementById("albums-search");return i&&(i.value=m),$(),w(),!0}let a=t.target.closest(".album-track-play");if(a){let i=a.closest(".album-track");if(i){t.stopPropagation();let n=i.dataset.ratingKey;await g(n,"music")}return!0}return!1}async function X(){let t=f();if(!t)return;document.title="Muziek \xB7 Albums";let e=await Z();if($(),t.innerHTML=`
    <div style="display: flex; gap: 8px; flex: 1;">
      <div style="flex: 1;" id="albums-container"></div>
      <div id="albums-az-rail" class="albums-az-rail"></div>
    </div>
