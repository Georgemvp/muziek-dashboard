import{b as H,c as K,d as G,e as O,f as _,g as N,k as q,l as D,m as R,n as S,o as V}from"./chunk-O4FREAIJ.js";import{a as Z}from"./chunk-FGEE3J4F.js";import{f as T,h as a,j as z}from"./chunk-NGNPS5HK.js";import"./chunk-2BMKGNH5.js";var p=1,h=null,u=new Set,g=new Set,b=1,C=!1,k=25,i=[],c="Mijn playlist",d=null,f=[],y=[],P=null,l=null,E=null,$=null,v="",F=null,J=null;async function Ie(){p=1,h=null,u=new Set,g=new Set,b=1,C=!1,k=25,i=[],c="Mijn playlist",d&&(d.abort(),d=null),f=[],y=[],P=null,l=null,E=null,$=null,v="",re(),Q()}function re(){if(document.getElementById("ms-playlist-styles"))return;let e=document.createElement("style");e.id="ms-playlist-styles",e.textContent=`
  ...
  `,Y())}function x(e){p=e;let t=document.querySelector(".ms-steps"),n=document.getElementById("ms-body");t&&(t.outerHTML=U()),n?(n.innerHTML=W(),Y(),document.querySelector(".ms-view")?.scrollIntoView({behavior:"smooth",block:"start"})):Q()}function oe(){return`
    <div class="ms-header">
  ...
  `}function U(){let e=[{n:1,label:"Prompt"},{n:2,label:"Filters"},{n:3,label:"Genereren"},{n:4,label:"Resultaat"},{n:5,label:"Acties"}];return`
    <nav class="ms-steps" aria-label="Stappen">
      ${e.map((t,n)=>`
        ...
        ${n<e.length-1?`<div class="ms-step-line${p>t.n?" done":""}"></div>`:""}
      `).join("")}
    </nav>
  ...
  `}function X(){return l?`
    <div class="ms-seed-chosen">
      ${l.ratingKey?`<img src="${S(l.ratingKey)}" class="ms-thumb" alt="" loading="lazy" decoding="async" onerror="this.remove()">`:`<div class="ms-thumb-ph" style="background:${z(l.title||"")}">\u266A</div>`}
  ...
  `:""}function ie(){let e=h?.genres||[],t=h?.decades||[],n=e.length?e.map(o=>`<button class="ms-chip${u.has(o)?" active":""}" data-chip-genre="${a(o)}">${a(o)}</button>`).join(""):'<span style="font-size:13px;color:var(--color-text-secondary)">Geen genres gesuggereerd</span>',s=t.length?t.map(o=>`<button class="ms-chip${g.has(o)?" active":""}" data-chip-decade="${a(o)}">${a(o)}</button>`).join(""):'<span style="font-size:13px;color:var(--color-text-secondary)">Geen decennia gesuggereerd</span>',r=Array(5).fill(0).map((o,m)=>m<b?"\u2605":"\u2606").join("");return`
    <div class="ms-card">
      <h2>Stel filters in</h2>

      <div class="ms-field">
        <label>Genres</label>
        <div class="ms-chips" id="ms-genre-chips">${n}</div>
      </div>

      <div class="ms-field">
        <label>Decennia</label>
        <div class="ms-chips" id="ms-decade-chips">${s}</div>
      </div>
  ...
  `}function ce(){let e=i.reduce((n,s)=>n+(s.duration||0),0),t=i.map((n,s)=>L(n,s,!0)).join("");return`
    <div class="ms-card">
      <input
        type="text"
  ...
  `}function de(){let e=f.length?f.map(n=>{let s=n.clientIdentifier||n.id||n.name||"",r=n.name||n.title||n.clientIdentifier||s;return`<option value="${a(s)}"${P===s?" selected":""}>${a(r)}</option>`}).join(""):'<option value="">Geen clients gevonden</option>',t=y.length?y.map(n=>{let s=n.ratingKey||n.id||"",r=n.title||n.name||s;return`<option value="${a(s)}">${a(r)}</option>`}).join(""):'<option value="">Geen playlists gevonden</option>';return`
  ...
  `}function L(e,t,n=!1){let s=e.ratingKey?S(e.ratingKey):null,r=s?`<img src="${a(s)}" class="ms-track-art" alt="" loading="lazy"
         onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`:"",o=`<div class="ms-track-art-ph"
    style="background:${z(e.album||e.title||"")};${s?"display:none":""}"
  >${T(e.album||e.title||"?")}</div>`,m=n?`<button class="ms-track-remove" data-remove-idx="${t}"
  ...
  `}function Y(){switch(document.getElementById("ms-back-btn")?.addEventListener("click",()=>{d&&(d.abort(),d=null),Z("mediasage")}),p){case 1:me();break;case 2:ge();break;case 3:be();break;case 4:xe();break;case 5:fe();break}}function me(){document.getElementById("ms-prompt")?.addEventListener("input",e=>{v=e.target.value;let t=document.getElementById("ms-analyze-btn");t&&(t.disabled=!v.trim()&&!l)}),document.getElementById("ms-seed-toggle")?.addEventListener("change",e=>{let t=document.getElementById("ms-seed-section");t&&(e.target.checked?t.classList.add("visible"):(t.classList.remove("visible"),l=null,t.innerHTML=`
        ...
        <div id="ms-seed-results"></div>
      `,j()))}),j(),ee(),document.getElementById("ms-analyze-btn")?.addEventListener("click",ue)}function j(){document.getElementById("ms-seed-search")?.addEventListener("input",e=>{clearTimeout(J);let t=e.target.value.trim();if(!t){let n=document.getElementById("ms-seed-results");n&&(n.innerHTML="");return}J=setTimeout(()=>pe(t),350)})}function ee(){document.getElementById("ms-seed-clear")?.addEventListener("click",()=>{l=null;let e=document.getElementById("ms-seed-section");e&&(e.innerHTML=`
        <input type="text" class="ms-text-input" id="ms-seed-search"
          placeholder="Zoek een nummer uit je bibliotheek\u2026" autocomplete="off">
        <div id="ms-seed-results"></div>
      `,document.getElementById("ms-seed-toggle").checked=!1,e.classList.remove("visible"),j())})}async function pe(e){let t=document.getElementById("ms-seed-results");if(t){t.innerHTML='<div class="loading" style="padding:10px;text-align:center"><div class="spinner"></div></div>';try{let n=await H(e),s=Array.isArray(n)?n:n?.tracks||n?.results||[];if(!s.length){t.innerHTML='<div style="padding:10px 12px;font-size:13px;color:var(--color-text-secondary)">Geen nummers gevonden</div>';return}let r=s.slice(0,12).map((o,m)=>{let I=o.title||o.name||"Onbekend",A=o.artist||o.grandparentTitle||"",B=o.album||o.parentTitle||"",se=o.ratingKey?`<img src="${S(o.ratingKey)}" class="ms-thumb" alt="" loading="lazy" decoding="async" onerror="this.remove()">`:`<div class="ms-thumb-ph" style="background:${z(B||I)}">${T(I)}</div>`;return`
      ...
      `}).join("");t.innerHTML=`<div class="ms-search-results">${r}</div>`,t.querySelectorAll(".ms-search-result").forEach(o=>{o.addEventListener("click",()=>{try{l=JSON.parse(o.dataset.track.replace(/&#39;/g,"'"))}catch{return}let m=document.getElementById("ms-seed-section");m&&(m.innerHTML=X(),ee())})})}catch(n){t.innerHTML=`<div class="error-box">Zoeken mislukt: ${a(n.message)}</div>`}}}async function ue(){let e=document.getElementById("ms-prompt"),t=(e?.value||v).trim();if(!t&&!l){e?.focus();return}let n=document.getElementById("ms-analyze-btn");n&&(n.disabled=!0,n.textContent="Analyseren\u2026");try{let s;l?s=await G({...l,prompt:t||void 0}):s=await K(t),h=s,v=t,u=new Set(s?.genres||[]),g=new Set(s?.decades||[]),x(2)}catch(s){n&&(n.disabled=!1,n.textContent="Analyseer \u2192"),ne(`Analyse mislukt: ${s.message}`)}}function ge(){document.getElementById("ms-genre-chips")?.addEventListener("click",e=>{let t=e.target.closest("[data-chip-genre]");if(!t)return;let n=t.dataset.chipGenre;u.has(n)?u.delete(n):u.add(n),t.classList.toggle("active",u.has(n)),w()}),document.getElementById("ms-decade-chips")?.addEventListener("click",e=>{let t=e.target.closest("[data-chip-decade]");if(!t)return;let n=t.dataset.chipDecade;g.has(n)?g.delete(n):g.add(n),t.classList.toggle("active",g.has(n)),w()}),document.getElementById("ms-rating-slider")?.addEventListener("input",e=>{b=parseInt(e.target.value);let t=document.getElementById("ms-rating-label");if(t){let n=Array(5).fill(0).map((s,r)=>r<b?"\u2605":"\u2606").join("");t.textContent=`${n} (${b}+)`}w()}),document.getElementById("ms-count-slider")?.addEventListener("input",e=>{k=parseInt(e.target.value);let t=document.getElementById("ms-count-val");t&&(t.textContent=k)}),document.getElementById("ms-exclude-live")?.addEventListener("change",e=>{C=e.target.checked,w()}),document.getElementById("ms-back-to-1")?.addEventListener("click",()=>x(1)),document.getElementById("ms-generate-btn")?.addEventListener("click",()=>x(3)),w(0)}function te(){return{prompt:v||void 0,seed_track:l||void 0,genres:[...u],decades:[...g],min_rating:b,exclude_live:C,track_count:k}}function w(e=420){clearTimeout(F),F=setTimeout(async()=>{let t=document.getElementById("ms-preview-count");if(t){t.innerHTML='<span class="ms-shimmer" style="width:38px;height:22px;vertical-align:middle;"></span>';try{let n=await O(te()),s=n?.count??n?.total??(typeof n=="number"?n:"\u2014"),r=document.getElementById("ms-preview-count");r&&(r.textContent=s)}catch{let n=document.getElementById("ms-preview-count");n&&(n.textContent="\u2014")}}},e)}function be(){i=[],E=null,$=null,d&&(d.abort(),d=null),d=V(te(),e=>{i.push(e);let t=document.getElementById("ms-stream-tracks");if(t){let s=document.createElement("div");s.innerHTML=L(e,i.length-1,!1),t.appendChild(s.firstElementChild)}let n=document.getElementById("ms-gen-text");n&&(n.textContent=`${i.length} track${i.length!==1?"s":""} gevonden\u2026`)},e=>{if(d=null,E=e.cost||e.estimated_cost||null,$=e.tokens||e.token_usage||null,e.tracks?.length&&!i.length){i=e.tracks;let r=document.getElementById("ms-stream-tracks");r&&(r.innerHTML=i.map((o,m)=>L(o,m,!1)).join(""))}if(c==="Mijn playlist"){let r=e.suggested_name||e.name||h?.suggested_name||h?.name;r&&(c=r)}let t=document.getElementById("ms-gen-text"),n=document.getElementById("ms-gen-spinner"),s=document.getElementById("ms-gen-done-row");t&&(t.textContent=`\u2713 Klaar \u2014 ${i.length} tracks`),n&&(n.style.animation="none",n.style.borderColor="var(--color-accent)"),s&&(s.style.display="flex",document.getElementById("ms-to-result-btn")?.addEventListener("click",()=>x(4)))},e=>{if(e?.name==="AbortError")return;ne(`Generatie mislukt: ${e.message}`);let t=document.getElementById("ms-gen-spinner");t&&(t.style.animation="none")})}function xe(){document.getElementById("ms-playlist-name")?.addEventListener("input",e=>{c=e.target.value}),document.getElementById("ms-result-tracks")?.addEventListener("click",e=>{let t=e.target.closest("[data-remove-idx]");if(!t)return;let n=parseInt(t.dataset.removeIdx);if(isNaN(n)||n<0||n>=i.length)return;i.splice(n,1);let s=document.getElementById("ms-result-tracks");s&&(s.innerHTML=i.map((r,o)=>L(r,o,!0)).join("")),ve()}),document.getElementById("ms-back-to-2")?.addEventListener("click",()=>x(2)),document.getElementById("ms-regen-btn")?.addEventListener("click",()=>{c=document.getElementById("ms-playlist-name")?.value||c,x(3)}),document.getElementById("ms-to-actions-btn")?.addEventListener("click",async()=>{c=document.getElementById("ms-playlist-name")?.value||c;let e=document.getElementById("ms-to-actions-btn");e&&(e.disabled=!0,e.textContent="Laden\u2026");try{let[t,n]=await Promise.all([q().catch(()=>[]),D().catch(()=>[])]);f=Array.isArray(t)?t:t?.clients||[],y=Array.isArray(n)?n:n?.playlists||[]}catch{f=[],y=[]}x(5)})}function ve(){let e=document.querySelector(".ms-meta");if(!e)return;let t=i.reduce((s,r)=>s+(r.duration||0),0),n=e.querySelectorAll("span");n[0]&&(n[0].textContent=`\u{1F3B5} ${i.length} tracks`),n[1]&&(n[1].textContent=`\u23F1 ${M(t)}`)}function fe(){document.getElementById("ms-play-btn")?.addEventListener("click",async()=>{let e=document.getElementById("ms-client-select")?.value;if(!e)return;P=e;let t=document.getElementById("ms-play-btn"),n=document.getElementById("ms-play-status");t&&(t.disabled=!0,t.textContent="Bezig\u2026");try{let s=i.map(r=>r.ratingKey).filter(Boolean);await R(s,e),t&&(t.disabled=!1,t.textContent="\u2713 Afspelen gestart"),n&&(n.textContent="\u2713")}catch(s){t&&(t.disabled=!1,t.textContent="\u25B6 Play Now"),n&&(n.textContent=`Mislukt: ${s.message}`)}}),document.getElementById("ms-save-btn")?.addEventListener("click",async()=>{let e=document.getElementById("ms-save-btn"),t=document.getElementById("ms-save-status");e&&(e.disabled=!0,e.textContent="Opslaan\u2026");try{await _({name:c,tracks:i,ratingKeys:i.map(n=>n.ratingKey).filter(Boolean)}),e&&(e.disabled=!1,e.textContent="\u2713 Opgeslagen"),t&&(t.textContent="\u2713 Playlist aangemaakt in Plex")}catch(n){e&&(e.disabled=!1,e.textContent="Opslaan in Plex"),t&&(t.textContent=`Mislukt: ${n.message}`)}}),document.getElementById("ms-add-btn")?.addEventListener("click",async()=>{let e=document.getElementById("ms-playlist-select")?.value;if(!e)return;let t=document.getElementById("ms-add-btn"),n=document.getElementById("ms-add-status");t&&(t.disabled=!0,t.textContent="Toevoegen\u2026");try{await N({id:e,ratingKeys:i.map(s=>s.ratingKey).filter(Boolean)}),t&&(t.disabled=!1,t.textContent="Toevoegen"),n&&(n.textContent="\u2713 Tracks toegevoegd")}catch(s){t&&(t.disabled=!1,t.textContent="Toevoegen"),n&&(n.textContent=`Mislukt: ${s.message}`)}}),document.getElementById("ms-back-to-4")?.addEventListener("click",()=>x(4))}function ne(e){let t=document.getElementById("ms-body");if(!t)return;let n=t.querySelector(".error-box.ms-injected");n&&n.remove();let s=document.createElement("div");s.className="error-box ms-injected",s.textContent=`\u26A0 ${e}`,t.insertBefore(s,t.firstChild)}function M(e){if(!e)return"";let t=Math.round(e/1e3),n=Math.floor(t/3600),s=Math.floor(t%3600/60),r=t%60;return n>0?`${n}u ${s}m`:`${s}:${String(r).padStart(2,"0")}`}export{Ie as loadMediaSagePlaylist};
