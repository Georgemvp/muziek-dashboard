// ── MediaSage landingspagina ─────────────────────────────────────────────────
// Native landingspagina met twee kaarten: AI Playlist & Album Recommend

import { switchView } from '../router.js';
import { getLibraryStats } from '../modules/mediasage-api.js';

/**
 * Verberg MediaSage UI — geen iframe meer, lege stub voor backward compat.
 */
export function hideMediaSageUI() {
  // Geen iframe meer; niets te verbergen
}

/**
 * Render de MediaSage landingspagina met status en twee navigatiekaarten.
 */
export async function loadMediaSage() {
  const content = document.getElementById('content');
  if (!content) return;

  // ── Laad library stats async ──────────────────────────────────────────────
  let statsHtml = '<span class="ms-status-loading">Verbinden met MediaSage…</span>';

  content.innerHTML = buildPage(statsHtml);
  bindEvents();

  try {
    const stats = await getLibraryStats();
    const trackCount = stats?.track_count ?? stats?.tracks ?? stats?.total_tracks ?? null;
    if (typeof trackCount === 'number') {
      statsHtml = `
        <span class="ms-status-dot ms-status-ok" aria-hidden="true"></span>
        <span class="ms-status-text">${trackCount.toLocaleString('nl-NL')} tracks gesynchroniseerd</span>`;
    } else {
      statsHtml = `
        <span class="ms-status-dot ms-status-warn" aria-hidden="true"></span>
        <span class="ms-status-text">Niet geconfigureerd</span>
        <button class="ms-sync-btn" id="ms-sync-btn">Synchroniseer</button>`;
    }
  } catch {
    statsHtml = `
      <span class="ms-status-dot ms-status-warn" aria-hidden="true"></span>
  ...
  const statusEl = content.querySelector('#ms-status');
  if (statusEl) {
    statusEl.innerHTML = statsHtml;
    const syncBtn = statusEl.querySelector('#ms-sync-btn');
    if (syncBtn) {
      syncBtn.addEventListener('click', () => {
        window.open('/mediasage/', '_blank', 'noopener');
      });
    }
  }
}
...
function bindEvents() {
  ...
  content.querySelector('#ms-card-playlist')?.addEventListener('click', () => {
    switchView('mediasage-playlist');
  });

  content.querySelector('#ms-card-recommend')?.addEventListener('click', () => {
    switchView('mediasage-recommend');
  });
}
