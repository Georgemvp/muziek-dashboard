import{a as w}from"./chunk-FNZ4ASA2.js";import{a as F,c as Q}from"./chunk-YH4MC2SR.js";import{d as M,h as o,j as J,z as h}from"./chunk-7DIT565V.js";import{a as G}from"./chunk-7DVASGHB.js";var S=["#1a237e","#283593","#3949ab","#5c6bc0","#7986cb","#9fa8da"],O=null;function at(e){let a=[...e];for(let s=a.length-1;s>0;s--){let t=Math.floor(Math.random()*(s+1));[a[s],a[t]]=[a[t],a[s]]}return a}typeof window<"u"&&!window._imgFb&&(window._imgFb=function(e,a){if(!e._d){e._d=1;var s=e.getAttribute("data-fb");if(s){e.src=s;return}}e.style.display="none",e.insertAdjacentHTML("afterend",'<div class="home-recent-cover-ph">'+(a||"\u266A")+"</div>")});function P(e,a,s,t,i,n){t=t||120,i=i||"\u266A";let r=e?M(e,t):null,l=a?"/api/imageproxy/artist/"+encodeURIComponent(a):null,d=r||l;if(!d)return'<div class="home-recent-cover-ph">'+i+"</div>";let y=n?' class="'+o(n)+'"':"",m=s?' alt="'+o(s)+'"':"",g=r&&l?' data-fb="'+o(l)+'"':"",u="_imgFb(this,'"+i+"')";return'<img src="'+o(d)+'"'+m+y+g+' loading="lazy" onerror="'+o(u)+'">'}function st(e){return!e||!Array.isArray(e)?{topartists:{artist:[]}}:{topartists:{artist:e.map(a=>({name:a.name,playcount:String(a.playcount||0),image:[null,null,{"#text":a.thumb||""},{"#text":a.thumb||""}],topTag:a.genre||null}))}}}function it(e){return!e||!Array.isArray(e)?{toptracks:{track:[]}}:{toptracks:{track:e.map(a=>({name:a.title,playcount:String(a.playcount||0),artist:{name:a.artist,"#text":a.artist},album:{"#text":a.album,name:a.album},image:[null,null,{"#text":a.thumb||""}]}))}}}function U(e){return!e||!Array.isArray(e)?[]:e.map(a=>({name:a.title,artist:{"#text":a.artist},album:{"#text":a.album},image:[null,null,{"#text":a.thumb||""}],date:{uts:String(a.viewedAt)}}))}function T(e){return e==null||isNaN(e)?"\u2014":Number(e).toLocaleString("nl-NL")}function Z(e){if(!e)return"";let a=new Date(e),s=Date.now()-a.getTime(),t=Math.floor(s/864e5);return t<1?"Vandaag":t===1?"Gisteren":t<7?`${t}d geleden`:t<31?`${Math.floor(t/7)}w geleden`:`${Math.floor(t/30)}mo geleden`}function C(e,a=120){return e?M(e,a):null}function gt(e,a,s){let t=e||"Muzikant",i=a?.artists??a?.artistCount??"\u2026",n=a?.albums??a?.albumCount??"\u2026",r=a?.tracks??a?.trackCount??"\u2026",l=a?.composers??a?.composerCount??"\u2014",d='<svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>',y='<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3"/><line x1="12" y1="3" x2="12" y2="9"/></svg>',m='<svg viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>',g='<svg viewBox="0 0 24 24"><path d="M9 12h6M9 8h6M9 16h4"/><rect x="3" y="4" width="18" height="16" rx="2"/></svg>',u=s?.ok?`<span style="width: 8px; height: 8px; background: #4caf50; border-radius: 50%; display: inline-block; margin-left: 8px; title='Last.fm connected'"></span>`:`<span style="width: 8px; height: 8px; background: #f44336; border-radius: 50%; display: inline-block; margin-left: 8px;" title='Last.fm unavailable'></span>`;return`
    ...
    </div>`}function bt(e){if(!e||e<1)return"0m";if(e<60)return`${Math.round(e)}m`;let a=Math.floor(e/60),s=Math.round(e%60);return s>0?`${a}h ${s}m`:`${a}h`}function nt(e,a){let s={};if(a&&Array.isArray(a))for(let u of a)u.date!=null&&(s[u.date]=u.minutes||(u.count?u.count*3.5:0));else for(let u of e||[]){let v=u.date?.uts;if(!v)continue;let f=new Date(parseInt(v,10)*1e3).toISOString().slice(0,10);s[f]=(s[f]||0)+3.5}let t=new Date,i=t.getDay(),n=new Date(t);n.setDate(t.getDate()-(i+6)%7),n.setHours(0,0,0,0);let r=[];for(let u=0;u<4;u++){let v=new Date(n);v.setDate(n.getDate()-u*7);let k=[];for(let f=0;f<7;f++){let L=new Date(v);L.setDate(v.getDate()+f);let B=L.toISOString().slice(0,10);k.push({key:B,minutes:s[B]||0})}r.push({days:k,totalMinutes:k.reduce((f,L)=>f+L.minutes,0)})}let l=Math.max(...r.map(u=>u.totalMinutes),1),d=Math.max(...r.flatMap(u=>u.days.map(v=>v.minutes)),1);function y(u){if(!u)return"width:6px;height:6px;background:#e0e0e0;";let v=u/d;return v<.25?"width:10px;height:10px;background:var(--accent);opacity:0.4;":v<.6?"width:16px;height:16px;background:var(--accent);opacity:0.7;":"width:24px;height:24px;background:var(--accent);opacity:1;"}let m=r.map(u=>{let v=u.totalMinutes>0?Math.round(u.totalMinutes/l*100):0,k=u.days.map(f=>`<div class="activity-dot-cell"><div class="activity-dot" style="${y(f.minutes)}" title="${f.key}: ${Math.round(f.minutes)}min"></div></div>`).join("");return`
    ...
    <div class="home-recent-artists-wrap">
      ...
      </div>`:""}catch(a){return console.warn("Recommended artists render mislukt:",a),""}}async function It(){try{let e=await h("/api/plex/playlists"),a=(e?.playlists||e||[]).slice(0,5);return a.length?`
    ...
    </div>`}function rt(e,a){let s=e?.releases||(Array.isArray(e)?e:[]),t=s.filter(v=>(v.type||"album").toLowerCase()!=="single"),i=s.filter(v=>(v.type||"").toLowerCase()==="single"),n=(a==="singles"?i:t).slice(0,3);if(!n.length)return'<div style="padding:32px;text-align:center;color:var(--text-muted);font-size:14px">Geen releases gevonden.</div>';let[r,...l]=n,d=C(r.image||r.thumb,400),y=d?`<img src="${o(d)}" alt="${o(r.title||r.album||"")}" loading="lazy">`:'<div class="releases-main-ph">\u266B</div>',m=r.description||r.bio||"",g=`
    ...
    </div>`,u=l.slice(0,2).map((v,k)=>{let f=C(v.image||v.thumb,160);return`
      <div class="releases-small-card">
        ${f?`<img src="${o(f)}" alt="${o(v.title||v.album||"")}" loading="lazy">`:'<div class="releases-small-ph">\u266B</div>'}
      ...
      <div class="home-wylbt-card">
        ...
        <div id="home-wylbt-releases-list">
          ${ct(a)}
            ...
            </div>`}).join("");t.innerHTML=n,t.querySelectorAll(".home-artist-circle-item").forEach(r=>{r.addEventListener("click",()=>{let l=r.dataset.artist;l&&w("artist-detail",{name:l})})})}}}catch(a){console.warn("Recent artists herlaad mislukt:",a)}}async function zt(){if(G.user?.name)return G.user.name;try{let e=await h("/api/user");return e?.user?.name||e?.name||null}catch{return null}}function dt(e){if(!e)return{};let a={...e};return e.artists!==void 0&&e.artistCount===void 0&&(a.artistCount=e.artists),e.albums!==void 0&&e.albumCount===void 0&&(a.albumCount=e.albums),e.tracks!==void 0&&e.trackCount===void 0&&(a.trackCount=e.tracks),a.artistCount!==void 0?a:e.library?dt(e.library):e.stats?e.stats:a}async function Wt(){let e=document.getElementById("content");if(!e)return;e.innerHTML=`
    <div class="home-page" aria-busy="true" aria-label="Laden\u2026">
      ${Q()}
      ${F(6,1)}
      ${F(4,2)}
    </div>`;let[a,s,t,i,n,r,l]=await Promise.all([zt().catch(()=>null),h("/api/plex/status").catch(()=>null),h("/api/plex/stats?period=7day").catch(()=>null),h("/api/wishlist").catch(()=>null),h("/api/releases").catch(()=>null),h("/api/user").catch(()=>null),h("/api/loved").catch(()=>null)]),d,y,m;t&&t.source==="plex"?(d=st(t.topArtists),y=it(t.topTracks),m={recenttracks:{track:U(t.recentTracks)}}):[d,y,m]=await Promise.all([h("/api/topartists?period=7day").catch(()=>null),h("/api/toptracks?period=7day").catch(()=>null),h("/api/recent?limit=200").catch(()=>null)]);let g={ok:r&&!r._stale,user:r?.user?.name||null},u=dt(s),v=m?.recenttracks?.track||[],k=i?.wishlist||i||[],f=n,L=l?.lovedtracks?.track||[],B=St(d),mt=await Et(d).catch(()=>""),ut=await At(d).catch(()=>""),vt=await It().catch(()=>""),z=v,N=null;if(t?.source==="plex"&&t?.dailyPlays?.some(c=>c.minutes>0||c.count>0))N=t.dailyPlays;else try{let c=await h("/api/activity?period=1month");c?.dailyPlays?.some(p=>p.minutes>0||p.count>0)&&(N=c.dailyPlays,c.recentTracks?.length&&(z=U(c.recentTracks)))}catch{try{z=(await h("/api/recent?limit=200"))?.recenttracks?.track||v}catch{}}e.innerHTML=`
    <div class="home-page">
